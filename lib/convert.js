...
/* Sportradar's match id, as index.html srOf reads it: an explicit srId from
   the API, or SportyBet's "sr:match:" prefix. Never a bare number - Bet9ja's
   own event ids are nine digits too. */
function srOf(x) {
  if (!x) return null;
  if (x.srId) return String(x.srId);
  const m = /^sr:match:(\d+)$/.exec(String(x.eventId || ""));
  return m ? m[1] : null;
}
...
function events(book, body) {
  ...
  return rows.map((x) => {
    ...
    const e = { id: x.eventId, home, away, ko, odds: x.odds || {} };
    const sr = srOf(x);
    if (sr) e.sr = sr;
    return e;
...
function pair(evs, leg) {
  ...
  /* The same Sportradar id inside the kickoff window is the same game. */
  const sr = srOf(leg);
  const byId = sr && near.find((e) => e.sr === sr);
  if (byId) return byId;
