...
/* The share card's address, carrying the build's content hash when there is
   one (see stampCard in scripts/prebuild.js): X, Telegram and WhatsApp cache a
   preview image per URL, so a new card needs a new address or the old one
   keeps showing on every shared page. */
function ogImage() {
  return ORIGIN + "/og-card.png" + (process.env.OG_V ? "?v=" + process.env.OG_V : "");
}
        ...
        ? "<p class='meta' style='margin:8px 0 0'>We tipped <strong>" + esc(tip) +
          ...
          (result.hit == null ? ""
            : "<span class='verdict " + (result.hit ? "hit" : "miss") + "'>" +
              (result.hit ? "Tip landed" : "Not this time") + "</span>")
        : "") +
      "</div>";
  } else if (tip) {
    head =
...
function renderCodesDay(entry, resultOf) {
  ...
  return staticPage({
    ...
    /* The books THIS day's code was minted on, read off the entry: early codes
       were SportyBet and Bet9ja only, later ones carry up to four. It is the
       link-preview line on X and Telegram, so it must not claim a book that
       day did not have (24 Sep). */
    desc: "The " + (() => {
        const N = { sporty: "SportyBet", bet9ja: "Bet9ja", betking: "BetKing", betpawa: "betPawa" };
        const b = Object.keys(entry.codes || {}).map((k) => N[k]).filter(Boolean);
        const list = b.length ? b : ["SportyBet"];
        return list.length > 1 ? list.slice(0, -1).join(", ") + " and " + list[list.length - 1] : list[0];
      })() + " booking code " + BRAND + " published on " +
      when + ", the " + (entry.legs || []).length + " games in it, and how each one finished.",
    h1: "Booking code for " + when,
    sub: headline + (sum && sum.all ? (sum.slip ? " - the slip won 🔥"
      : sum.hit >= sum.of - 1 ? " - so close 😤" : " - on to the next one 💪") : ""),
...
function renderCodesHub(entries, resultOf) {
  ...
  return staticPage({
    ...
    desc: "Today's booking code for SportyBet, Bet9ja, BetKing and betPawa, the " +
      "games in it, and how every previous day's code finished. Graded, not just posted.",
...
function xResultPost(e, resultOf) {
  const s = codeSummary(e.legs, resultOf);
  /* UPBEAT, NEVER UNTRUE (24 Sep, the owner's call). It opened on a red ❌,
     which read as "we lost" before anyone saw 11 of 12. The head leads with
     the energy and the real count; the per-leg ✅/❌ below stay, because
     showing every result is the whole reason this account is believed. */
  const day = xDay(e.date) + "'s code";
  const head = (s.slip
    ? "✅ " + day + " WON 🔥🧙 " + s.hit + " of " + s.of + " landed."
    : s.hit >= s.of - 1
      ? "😤 So close. " + day + ": " + s.hit + " of " + s.of + " landed, one leg off."
      : "📊 " + day + ": " + s.hit + " of " + s.of + " landed. New code is up, we go again 💪") + "\n\n";
...
<p><b>${rows.length}</b> codes posted · <b>${n("won")}</b> won 🔥 · <b>${n("lost")}</b> not this time · ${n("pending")} still playing</p>
<p class="meta">${rows.slice(0, 30).map((r) => esc(xDay(r.date)) + ": " +
    (r.state === "pending" ? "playing" : (r.state === "won" ? "✅ won" : "😤 not this time") + " (" + r.s.hit + " of " + r.s.of + ")")).join("<br>")}</p></section>`;
}
