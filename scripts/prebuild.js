...
async function bakePayload() {
  ...
  (buildLog || [])
    ...
    .filter((l) => /held back|unavailable|failed|oracle|soccervista|suspended|committed floor|country offsets|record:|recorded result|score source|FT score map|backfill|harvested results|of \d+ confirmed|^\S+ \d{4}-\d{2}-\d{2}:|^downloaded \d+\/\d+|^skip \S+\.csv|^corners|^shots/i.test(l))
    .forEach((l) => log(l));
