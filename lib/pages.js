...
function renderConvertPage() {
  return staticPage({
    ...
    body: `
<div class="prose">
  <p>A booking code only works at the bookmaker that issued it. A SportyBet code
  pasted into Bet9ja is not a slip, it is a typo - which is the single most
  common reason a code "does not load".</p>

  <p>${BRAND} reads the slip behind a code and books the same games at the other
  bookmaker, so what comes back is a code that book will take. Paste the code
  into the box on the <a href="/">predictions page</a> and press
  <b>Convert</b>.</p>

  <h2>What happens to your slip</h2>
  <ol>
    <li>We ask the bookmaker what is behind the code. What comes back is what
    they hold <em>today</em>, which can be shorter than what was booked - they
    drop games from a coupon on their own schedule, and neither book says how
    many there were.</li>
    <li>Each game is matched to our board, and the board already knows both
    bookmakers' id for it. Nothing is guessed by name at this point.</li>
    <li>The market is translated, and the slip is booked at the other book. You
    get its own code.</li>
  </ol>

  <h2>What gets left behind, and why</h2>
  <ul>
    <li><b>A game that has kicked off.</b> Neither book will take it, and one
    such leg refuses the whole ticket, so it comes out before we ask.</li>
    <li><b>A game the other bookmaker does not list.</b> Coverage is not
    identical; Bet9ja carries about 97% of what SportyBet does.</li>
    <li><b>A game that is not on our board.</b> Every board is a day's fixtures,
    so a code built a week ago will have games we no longer carry.</li>
    <li><b>A market only one of them sells.</b> Asian handicap quarter lines -
    +0.25, -1.75 - exist at Bet9ja and nowhere on SportyBet's card, which quotes
    halves and wholes from -4.5 to 5. Corners run 6.5 to 12.5 at SportyBet and
    7.5 to 14.5 at Bet9ja, so the ends have nowhere to land.</li>
  </ul>
  <p>Whatever is left behind is named on screen, game by game, before you book
  anything.</p>

  <h2>The lines that cannot travel unchanged</h2>
  <p>Two families are sold at lines that do not overlap, so the bet itself has
  to move for the leg to cross:</p>
  <ul>
    <li><b>Whole Over/Under lines.</b> SportyBet sells Over 2 and Over 3; Bet9ja
    stops at the half lines. Over 2 returns your stake on exactly two goals
    where Over 1.5 has already won, so these are <em>not</em> the same bet. The
    leg moves to the nearest line Bet9ja sells and every screen that shows it
    says it was changed.</li>
    <li><b>1X2 or Over/Under.</b> SportyBet sells it only at 2.5, Bet9ja only at
    1.5 and 3.5. Moving one is a whole goal, not a half, so it is offered with a
    switch rather than done for you - and only from Bet9ja to SportyBet, where
    each of their lines has exactly one place to go. The other direction would
    mean picking between 1.5 and 3.5 on somebody else's money, so it is
    refused.</li>
  </ul>

  <h2>What a converted slip is worth</h2>
  <p>The price is the one the bookmaker taking it publishes, not the one on the
  code you pasted. Where that book has not published a price for a leg - a
  ...
  <p>A converted slip is not the same bet as the one you pasted. It is the same
  games, at another bookmaker's prices, minus whatever could not travel.</p>

  <h2>Markets that cross untouched</h2>
  <p>Everything we predict - 1X2, double chance, over and under the half lines,
  both teams to score, team totals - plus a number we do not predict but do
  move: Asian handicap on the shared lines, total corners, team cards, draw no
  bet, win either half, 1UP and 2UP, and 1X2 or GG.</p>

  <h2>Reading a code without converting it</h2>
  <p>The same box shows you what is behind any code from either book - the
  games, the markets and the bookmaker's own odds - and can cut a long slip into
  several smaller tickets. Reading costs nothing and books nothing.</p>
</div>
