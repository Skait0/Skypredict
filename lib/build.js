...
/* Explicit, though Node 24 happens to expose `zlib` as a global and the code
   ran without it here. package.json allows node >=20, where it is not global,
   so leaning on that would have worked locally and broken on the runtime
   Vercel actually picked. */
...
const CORN = require("./corners.js");
const INTL = require("./internationals.js");
const EUROFF = require("./euroffsets.js");
const LIVE = require("./liveresults.js");
/* LAGOS_OFFSET_MS, because "today" is the reader's day and the readers are in
   Lagos. Taken from quota.js rather than restated: two definitions of the day
   boundary is how the headline ended up on a game the reader's own calendar
   calls tomorrow. */
...
function scoreForTip(k, tipLabel, seed) {
  ...
  /* AN UPSET NEEDS A HIGHER BAR THAN A DRAW, and the two were sharing one.
     Reported on the live pick of the day, 7 Sep 2026: Midtjylland v
     Nordsjaelland, 50/23/26, published 0-2. Away cleared 0.35 comfortably - it
     was 52% as likely as home - so away scorelines stayed in the pool, and
     19.5% of seeds on that fixture printed one.
     A draw beside a 50% favourite reads as "tight game". The underdog WINNING
     beside 50/26 reads as the board disagreeing with itself, in the most
     prominent slot on the site. So the losing side has to be genuinely close
     before its scoreline is printed - 0.70, which keeps the 41/28/31 fixture
     the original rule was written around (away is 0.76 of the favourite there)
     and drops this one (0.52). The draw floor is untouched, deliberately:
     raising it is what made draws vanish the first time. */
  const UPSET_FLOOR = 0.70;
  ...
  if (oc.home != null && oc.draw != null && oc.away != null) {
    ...
    if (best > 0) {
      /* A side is judged against the OTHER side, not against the best of all
         three - the question is whether these two teams are close, and a high
         draw share should not make an upset look more plausible than it is. */
      const sideBest = Math.max(oc.home, oc.away);
      const live = {
        home: oc.home >= best * OUTCOME_FLOOR &&
              (oc.home >= sideBest || oc.home >= sideBest * UPSET_FLOOR),
        draw: oc.draw >= best * OUTCOME_FLOOR,
        away: oc.away >= best * OUTCOME_FLOOR &&
              (oc.away >= sideBest || oc.away >= sideBest * UPSET_FLOOR),
      };
  ...
  /* THE SAME COHERENCE PROMISE, APPLIED TO GOALS.
   *
   * The gate above stops the scoreline contradicting the fixture's own
   * home/draw/away numbers. Nothing was stopping it contradicting the goals
   * numbers, and the board publishes those just as prominently.
   *
   * Reported twice by the owner - "barca predicts 0-0 at 69 percent" - and it
   * was not one bad fixture. Measured on the live board, 280 scorelines:
   *
   *   54 of them (19%) showed 0 or 1 goal on a fixture the SAME ROW called
   *   70%+ for over 1.5. Chelsea v Hull printed 0-0 beside "78% over 1.5" and
   *   an expected 2.85 goals. Everton v Man United, Man United v Man City,
   *   Liverpool v Fulham: all 1-0 against a 75-78% over-1.5 call.
   *
   * The cause is DRAW_WEIGHT below meeting a Poisson distribution. Draws are
   * damped to 0.60 to correct a real selection effect, which pushes 1-1 under
   * 1-0 - and 1-0 is a one-goal game on a card that expects nearly three.
   *
   * So: when the fixture itself makes two goals likely, the scoreline has to
   * agree. Relative to nothing, because this is not a claim about how many
   * goals get scored - it is the board being made to say one thing at a time.
   *
   * 0.70 rather than the 0.80 the tip uses to headline Over 1.5. The headline
   * bar answers "is this safe enough to back"; this one answers "would a
   * reader call these two numbers a contradiction", and they would well below
   * the point where we would bet on it.
   *
   * Applied AFTER the tip filter and never allowed to empty the pool, exactly
   * like the gate above: where the two disagree the tip still wins, because a
   * scoreline that loses its own tip is the worse contradiction.
   */
  const O15_COHERENT = 0.70;
  if (typeof k.o15 === "number" && k.o15 >= O15_COHERENT) {
    const kept = pool.filter((c) => {
      const [h, a] = c.s.split("-").map(Number);
      return (h + a) >= 2;
    });
...
/* THE TWO VHOSTS FAIL INDEPENDENTLY, AND ONE OF THEM WAS DOWN FOR THREE DAYS.
 *
 * From 5 Sep 2026 every request here answered 503 with `Retry-After` and
 * `X-WS-Origin: available` - the edge shedding load while the origin was
 * fine. The board ran on the committed floor for three days and the model
 * aged a day per day.
 *
 * It was never the site. `www.football-data.co.uk` was 503ing while
 * `football-data.co.uk` served every file normally, on the SAME IP
 * (217.160.0.246) - so it was one vhost, not the host. Checked across all
 * eight shapes the build asks for, main and per-country, fixtures and the
 * combined results file.
 *
 * So: no www. And if this ever 503s again, try the other host before
 * concluding anything - it costs one curl and it is what three days of
 * degraded builds turned out to be. */
...
/* ONE CLUB, ONE KICK-OFF. Records this fixture's two clubs against its
   kick-off instant and answers with the fixture already holding that slot, or
   null when the slot is free.
 *
 * On the instant rather than the date, because a double-header at different
 * times is a real thing and two matches at the same minute sharing a club is
 * not. Keyed on the MATCHED team rather than the feed's spelling, which is the
 * whole point: the pair this was written for differed only in the away name -
 * "Barracas Central v Ind. Rivadavia" against "Barracas Central v
 * Independiente", 21 Sep 2026, both 22:00, both real clubs - so every
 * name-keyed guard passed them and the board published a match that does not
 * exist.
 *
 * Mutates `seen`, which is why it takes it: the caller is a streaming loop
 * with several other reasons to skip a row, and a second pass over the kept
 * rows would have to repeat every one of them. */
function slotClash(seen, slotIso, teams, name) {
  for (const team of teams) {
    const held = seen[slotIso + "|" + team];
    if (held) return held;
  }
  for (const team of teams) seen[slotIso + "|" + team] = name;
  return null;
}
...
/* WHICH SEASONS TO DOWNLOAD, worked out rather than written down.
 *
 * football-data labels a season by the July it starts in: 2026-27 is "2627",
 * at /mmz4281/2627/F1.csv. The list used to be the literal ["2425","2526"],
 * and on 2 September 2026 the build was still asking for those two and being
 * served them perfectly - so `downloaded 60/60 sources` was true while every
 * European league's CURRENT season was missing entirely.
 *
 * Nothing could catch it. The model was fitted on results ending in May 2026,
 * with a 200-day half-life, and the only visible symptom was two Ligue 1 and
 * Primeira Liga results held back for want of a score that was sitting in a
 * file nobody asked for. The per-country files (ARG, BRA, USA...) are
 * calendar-year and not season-coded, which is why they kept working and hid
 * how much was gone.
 *
 * A hardcoded list is a bug with a delayed fuse - it works until July and then
 * silently stops. Computed, the turn of a season is not a code change.
 *
 * The turn is July, before any league kicks off, so the current season is
 * never missed. For the week or two before the new file is published a request
 * for it 404s; that is non-fatal by design and now prints as `skip F1.csv`. */
function seasonCodes(nowMs, count) {
  const d = new Date(nowMs);
  /* A season is named for the calendar year it begins in, and it begins in
     July - so up to June we are still inside the season that began last
     year. */
  const start = d.getUTCMonth() >= 6 ? d.getUTCFullYear() : d.getUTCFullYear() - 1;
  const two = (y) => String(((y % 100) + 100) % 100).padStart(2, "0");
  const out = [];
  for (let i = count - 1; i >= 0; i--) out.push(two(start - i) + two(start - i + 1));
  return out;
}

const DEFAULTS = {
  /* Three, not the two it was: the current season plus two of history, so
     adding the missing one takes nothing away from the model. */
  seasons: seasonCodes(Date.now(), 3),
  halfLife: 200,
  shrinkage: 35,
  daysAhead: 21,
  minLeagueMatches: 60,
...
/* A COMMITTED FLOOR UNDER THE RESULTS DOWNLOAD.
 *
 * On 5-6 Sep 2026 football-data.co.uk answered 503 to everyone for hours. The
 * results guard below refuses under 400 matches - correctly, a half-downloaded
 * history would refit the model on nothing - so the build threw. And because it
 * throws BEFORE the fixture feeds are read, the whole site went dark over a
 * source we only need for HISTORY. Today's card was never the problem: the
 * SportyBet feed was healthy throughout and carries the large majority of it.
 *
 * The thing that makes this fixable is that finished seasons never change.
 * 2024-25 and 2025-26 are closed; their files are the same bytes today as they
 * will be in a year. So they are committed, and read from disk whenever the
 * download for that file fails.
 *
 * What this buys: an outage costs a few weeks of the CURRENT season's form,
 * not the site. The model still fits, the fixtures are still today's, and the
 * board still publishes.
 *
 * It also cuts what we take from a free, volunteer-run dataset by two thirds -
 * the same dataset the CPU incident hit 47,000 times in twelve hours. Two
 * seasons of every division no longer need asking for at all.
 *
 * Read-only on purpose. A Vercel build is ephemeral, so writing back here would
 * be thrown away; refreshing the floor is a deliberate commit, which is right
 * for something whose whole value is that it does not change.
 */
const RESULTS_CACHE = path.join(__dirname, "..", "data", "results");
function cachedResultsFor(src) {
  try {
    let name = null;
    if (src.kind === "main") {
      const m = /\/mmz4281\/(\d{4})\/([A-Za-z0-9]+)\.csv$/.exec(src.url || "");
      if (m) name = m[1] + "_" + m[2] + ".csv";
    } else {
      const m = /\/new\/([A-Za-z0-9]+\.csv)$/.exec(src.url || "");
      if (m) name = "extra_" + m[1];
    }
    if (!name) return null;
    /* Stored gzipped: these are CSVs, so it is 14 MB of repo against 3.3 MB,
       and every clone on every build pays that difference. Plain .csv is still
       read if present, so a file can be dropped in uncompressed to test one. */
    const gz = path.join(RESULTS_CACHE, name + ".gz");
    if (fs.existsSync(gz)) {
      const text = zlib.gunzipSync(fs.readFileSync(gz)).toString("utf8");
      return text && text.length > 100 ? text : null;
    }
    const p = path.join(RESULTS_CACHE, name);
...
function loadFloorMatches() {
  ...
  return out.concat(loadLiveMatches());
}

/* THE CURRENT SEASON, HARVESTED WHILE FOOTBALL-DATA IS DOWN.
 *
 * The floor above holds finished seasons, which never change and therefore
 * never need refreshing. This holds the season in progress, which changes
 * every week and is exactly what an outage costs us - see lib/liveresults.js
 * for where the rows come from and why every club name in them is ours.
 *
 * Written by scripts/mkresults.js and committed, for the same reason as the
 * floor: a Vercel build is ephemeral, so there is nowhere else for it to live.
 *
 * Leagues are filtered again on the way in, not trusted from the file. The
 * file is generated, and a generated file with a league we hold no ratings
 * for would put unrated clubs into the fit.
 */
/* CORNERS AND SHOTS FOR MATCHES WE ALREADY HAVE.
 *
 * football-data's extra files - MLS, Liga MX, Argentina, the Nordic top
 * flights - give goals and nothing else. data/results/stats_apif.csv.gz
 * (scripts/apifbackfill.js --overlay) holds API-Football's statistics for the
 * same games, under our club names. It is an overlay, never a result: each row
 * only fills the statistics of a match already here, same league and clubs,
 * within a day either way - the two feeds date a late kick-off differently, and
 * adding it as a row would fit that game twice. A match that already carries a
 * figure keeps its own. Returns how many were filled. */
function applyStatsOverlay(matches, log, rows) {
  if (!rows) {
    rows = [];
    try {
      const f = path.join(RESULTS_CACHE, "stats_apif.csv.gz");
      if (fs.existsSync(f)) rows = LIVE.fromCSV(zlib.gunzipSync(fs.readFileSync(f)).toString("utf8"));
    } catch (e) { (log || []).push(`stats overlay unreadable: ${e.message}`); return 0; }
  }
  if (!rows.length) return 0;
  const by = new Map();
  for (const r of rows) by.set(`${r.league}|${r.home}|${r.away}|${r.date}`, r);
  const KEYS = ["hc", "ac", "hs", "as", "hst", "ast"];
  let filled = 0;
  for (const m of matches) {
    if (!m.date || m.hc != null) continue;
    const t = m.date.getTime();
    let r = null;
    for (const d of [0, -1, 1]) {
      const day = new Date(t + d * 86400000).toISOString().slice(0, 10);
      r = by.get(`${m.league}|${m.home}|${m.away}|${day}`);
      if (r) break;
    }
    if (!r) continue;
    for (const k of KEYS) if (m[k] == null && r[k] != null) m[k] = r[k];
    filled++;
  }
  if (log) log.push(`stats overlay: corners and shots filled on ${filled} of ${rows.length} rows`);
  return filled;
}
...
function loadLiveMatches() {
  const allowed = new Set([].concat(
    Object.values(MAIN),
    Object.keys(EXTRA).map((c) => c + " " + EXTRA[c]),
    LIVE.HARVEST_EXTRA));
  const out = [];
  let names;
  try { names = fs.readdirSync(RESULTS_CACHE); } catch (e) { return out; }
  for (const name of names) {
    if (!/^live_[A-Za-z0-9-]+\.csv(?:\.gz)?$/.test(name)) continue;
    let text = null;
    const full = path.join(RESULTS_CACHE, name);
    try {
      text = name.endsWith(".gz")
    ...
    const res = M.normalise(rows);
    if (res.error) continue;
    for (const r of res.matches) if (allowed.has(r.league)) out.push(r);
  }
  return out;
}
...
const TIER_HANDICAP = {
  ...
  /* The warming second tiers. 0.26 is the median of the five measured above
     rather than anything observed in Denmark, Norway or the Netherlands - we
     have no cross-division results there and will not have any, since no club
     plays in two leagues. It is the same class of assumption as the numbers
     above it and it is marked the same way: every fixture priced through it
     is cross-tier, shrunk by CROSS_TIER_SHRINK, and barred from the pick of
     the day. */
  "Netherlands Eredivisie": 0.00, "Netherlands Eerste Divisie": 0.26,
  "Denmark Superliga": 0.00,      "Denmark 1. Division": 0.26,
  "Norway Eliteserien": 0.00,     "Norway 1st Division": 0.26,

  /* The second wave, on the same footing and under the same caveat: carried
     over from the measured ladders above, not observed in these countries.
     A second tier is 0.26, as in Spain, Italy and France. A third tier adds
     the step Scotland measured between its second and third (0.18), giving
     0.44 - Germany's 3. Liga takes Scotland's own 0.42, because Germany's
     second-tier gap (0.24) is Scotland's rather than Spain's. Every fixture
     priced through one of these is cross-tier, shrunk and barred from the
     pick of the day, as the note above says. */
  "Brazil Serie A": 0.00,              "Brazil Brasileiro Serie B": 0.26,
  "Argentina Liga Profesional": 0.00,  "Argentina Primera Nacional": 0.26,
  "Austria Bundesliga": 0.00,          "Austria 2. Liga": 0.26,
  "Greece Super League": 0.00,         "Greece Super League 2": 0.26,
  "Slovakia Nike liga": 0.00,          "Slovakia 2. Liga": 0.26,
  "Bulgaria efbet League": 0.00,       "Bulgaria Vtora Liga": 0.26,
  "Sweden Allsvenskan": 0.00,          "Sweden Superettan": 0.26,
  "Mexico Liga MX": 0.00,              "Mexico Liga de Expansion MX": 0.26,
  "USA MLS": 0.00,                     "USA USL Championship": 0.26,
  "Germany 3. Liga": 0.42,
  "France Ligue 3": 0.44,
  "Italy Serie C, Group A": 0.44, "Italy Serie C, Group B": 0.44,
  "Italy Serie C, Group C": 0.44,
  "Spain Primera Federacion": 0.44,

  /* The third wave (25 Sep), on the second wave's footing and caveat: a
     second tier 0.26, a third 0.44, carried over, not observed. */
  "Japan J2 League": 0.26,        "Ireland First Division": 0.26,
  "Finland Ykkosliiga": 0.26,     "Russia 1. Liga": 0.26,
  "Egypt 2. Division A": 0.26,     "Northern Ireland Championship": 0.26,
  "Brazil Brasileiro Serie C": 0.44, "Argentina Primera B": 0.44,
  "Denmark 2nd Division": 0.44,
};
...
/* Harder still across a border. A domestic cup tie leans on a ladder shaped
   from published rating separations between divisions of the SAME league
   system; a European tie leans on a coefficient bridging two of them, and on
   a country-level offset that cannot know Celtic from the rest of Scotland.
   More assumption, less confidence. Barred from pick of the day either way. */
const CROSS_COUNTRY_SHRINK = 0.55;
 ...
 */
/* ------------------------------------------------ ACROSS A BORDER, NOT JUST
                                                     ACROSS A DIVISION

   The ladder above is per country, so tierEdge returned null for every
   European tie and the board refused the lot: Real Madrid v Inter, Napoli v
   Arsenal, Bayern v Bodoe/Glimt, 28 of them in one round. Refusing was the
   right behaviour - it is the alternative to quietly declaring two clubs even
   because we cannot compare them - but it meant going silent on the most
   watched football of the week.

   NOTHING IN OUR OWN DATA CAN SETTLE THIS, and that was checked rather than
   assumed. The training results cover 26 countries and 66,965 matches, and:
   no match crosses a border, and no club appears in two countries' leagues.
   With no observation linking one country's scale to another's, the offset
   between Spain and Italy is not merely unknown, it is unidentifiable. Fitting
   it is not an option; it has to be anchored from outside.

   THE ANCHOR IS THE UEFA ASSOCIATION COEFFICIENT, which is the one published
   number built for exactly this question - how a country's clubs actually fare
   against other countries' clubs, over five seasons. It is not our estimate,
   it is not a guess about form, and it updates once a year.

   The conversion is deliberate and is the part to argue with:

     strength gap = COUNTRY_SCALE x (ln(England) - ln(country))

   Log, because coefficients are roughly multiplicative - the gap from 100 to
   50 is the same kind of step as 50 to 25, not the same as 50 to 0.
   COUNTRY_SCALE = 0.50 puts England to Portugal at 0.23 and England to the
   Netherlands at 0.33, either side of the 0.28 that one English division
   costs, which is the right order for those leagues. England to Norway lands
   at 0.49, about two divisions.

   COUNTRY_CAP exists because log scaling runs away in the tail: Wales would
   come out at 1.56, nearly four English divisions, which is beyond anything
   this model was fitted to represent. 0.70 is a shade under the widest step
   the domestic ladder carries (Premier League to Conference National, 0.77).

   WHAT THIS IS NOT. It is a country-level offset, so it moves a whole league
   at once and cannot know that Celtic are stronger than Scotland's coefficient
   suggests, or that Bodoe/Glimt are stronger than Norway's. The club's own
   rating still applies on top of it, but within its own league only. That
   limitation is why these ties get goals markets and nothing else - see
   crossCountry in bestTip.

   Non-UEFA countries get no coefficient and keep being refused. Argentina and
   Brazil are in our ratings and the feed carries Libertadores and Sudamericana
   ties, but CONMEBOL has no equivalent published number that is comparable to
   this one, and inventing a bridge between the two confederations is exactly
   the sort of number this comment exists to avoid. */
const UEFA_COEFFICIENT = {
  /* Five-season association coefficients, 2022/23 to 2026/27, as published
     27 Aug 2026. Only the countries our ratings cover are listed; the rest of
     the 55 would never be consulted. Refresh once a season - the shape barely
     moves, but the numbers do. */
  England: 102.019, Italy: 87.874, Spain: 82.493, Germany: 80.402,
  France: 68.153,   Portugal: 64.550, Belgium: 59.050, Netherlands: 52.395,
  Turkey: 49.675,   Poland: 45.125, Greece: 43.812, Norway: 38.612,
  Denmark: 38.306,  Switzerland: 30.200, Austria: 27.650, Scotland: 26.250,
  Sweden: 25.750,   Romania: 24.875, Russia: 17.332, Ireland: 16.343,
  Finland: 13.625,

  /* THE OTHER SIDE OF EVERY EUROPEAN NIGHT.
     The thirty warming leagues in HARVEST_EXTRA are the top flight of the UEFA
     countries we did not rate, and this function refuses a country with no
     entry here BEFORE it looks at any fitted offset - so harvesting their
     results is necessary and not sufficient. Same table, same publication:
     five-season association coefficients 2022/23 to 2026/27, as published
     27 Aug 2026. Keyed on the first word of the league name, which is what
     countryOfLeague reads, so "Bosnia" rather than "Bosnia and Herzegovina".
     Most of these sit far enough down the table that the arithmetic runs past
     COUNTRY_CAP - see the refusal below. They are listed anyway, because the
     fitted offsets that will eventually place them are keyed on countries this
     table admits. */
  Czechia: 44.725,  Cyprus: 34.193, Hungary: 27.312, Croatia: 25.656,
  Ukraine: 24.087,  Israel: 23.875, Azerbaijan: 23.687, Slovenia: 22.843,
  Slovakia: 22.375, Bulgaria: 20.562, Serbia: 18.250, Iceland: 17.020,
  Armenia: 15.812,  Kosovo: 14.031, Bosnia: 13.968, Latvia: 13.875,
  Kazakhstan: 12.375, Moldova: 10.250, Albania: 9.000, Belarus: 8.708,
  Lithuania: 8.125, Malta: 7.625, Estonia: 7.041, Georgia: 6.500,
  Luxembourg: 6.250, Montenegro: 5.833, Wales: 4.499,
  /* Added 25 Sep 2026 with the NI Premiership (kassiesa.net crank2027, whose
     Estonia and Ireland agree with the figures above to the third decimal).
     Keyed on the whole name - model.js countryOf reads it whole. */
  "Northern Ireland": 6.625,
};
const COUNTRY_ANCHOR = 102.019;   /* England, the top of the table */
const COUNTRY_SCALE = 0.50;
const COUNTRY_CAP = 0.70;
...
function countryHandicap(country) {
  ...
  /* MEASURED, AND THE MEASUREMENT RAN OUT OF ROOM. A country the fit pushed
     onto COUNTRY_CAP has not been placed, it has been bounded - and the
     corpus says by a long way: Irish clubs score less than half what we
     predict even after the fit. Refusing is the same answer this function has
     always given for a country with no coefficient, and it is the honest one:
     the alternative is publishing a number we know to be wrong by a factor
     the cap is hiding. */
  if (EUROFF.isPinned(country)) return null;
  ...
  /* THE SAME RULE THE FIT OBEYS, ON THE WAY IN.
     A fitted offset that lands on the cap is refused above, because the cap
     means we ran out of room to describe the gap rather than measured it. The
     imported arithmetic can run past the cap for exactly the same reason - log
     scaling runs away in the tail, and a coefficient of 4.5 asks for 1.56,
     more than twice the widest step the domestic ladder carries. Clamping that
     to 0.70 publishes a lower bound wearing an estimate's clothes; every
     country in the table when this rule was written sat under it, so this
     changes nothing that shipped and gates everything added since. */
  if (raw >= COUNTRY_CAP) return null;
...
const LEAGUE_TIER = {
  ...
  /* Warming, from the harvest rather than from football-data - see
     HARVEST_EXTRA in lib/liveresults.js. Listed before they have any results,
     which costs nothing: with no clubs in the index no fixture can reach
     these rows, and it means the ladder is not the thing forgotten on the day
     the league finally crosses minLeagueMatches. */
  "Netherlands Eerste Divisie": 2, "Denmark 1. Division": 2,
  "Norway 1st Division": 2,

  /* The UEFA top flights, warming - see HARVEST_EXTRA in lib/liveresults.js.
     Each is its country's first division, so the rung is zero by definition
     and rungOf answers without a TIER_HANDICAP entry. Listed before they
     have a single result, which costs nothing: with no clubs in the index no
     fixture can reach them, and it means the ladder is not the thing
     forgotten on the day one of them crosses minLeagueMatches. */
  "Czechia Chance Liga": 1,
  "Slovakia Nike liga": 1,
  "Ukraine Premier League": 1,
  "Croatia HNL": 1,
  "Serbia Mozzart Bet Super Liga": 1,
  "Israel Ligat ha'Al": 1,
  "Cyprus Cyprus League": 1,
  "Bulgaria efbet League": 1,
  "Slovenia Prva liga": 1,
  "Hungary NB I.": 1,
  "Azerbaijan Premier League": 1,
  "Kazakhstan Premier League": 1,
  "Georgia Crystalbet Erovnuli Liga": 1,
  "Iceland Besta deild karla": 1,
  "Belarus Vysshaya Liga": 1,
  "Bosnia WWIN Liga": 1,
  "Latvia Virsliga": 1,
  "Lithuania TOPLYGA": 1,
  "Estonia Meistriliiga": 1,
  "Armenia Premier League": 1,
  "Albania Abissnet Superiore": 1,
  "Moldova Super Liga": 1,
  "Malta Premier League": 1,
  "Luxembourg BGL Ligue": 1,
  "Wales Cymru Premier": 1,
  "Montenegro Prva Crnogorska Liga": 1,
  "Kosovo Superliga": 1,

  /* The second wave - see HARVEST_EXTRA. Three top flights, and the second
     and third tiers of countries whose top flight we already rate. */
  /* National teams, one pool - see lib/internationals.js. */
  "International": 1,
  "Colombia Liga DIMAYOR": 1, "Uruguay Primera Division": 1,
  "Chile Primera Division": 1,
  "Brazil Brasileiro Serie B": 2, "Argentina Primera Nacional": 2,
  "Austria 2. Liga": 2,           "Greece Super League 2": 2,
  "Slovakia 2. Liga": 2,          "Bulgaria Vtora Liga": 2,
  "Sweden Superettan": 2,         "Mexico Liga de Expansion MX": 2,
  "USA USL Championship": 2,
  "Germany 3. Liga": 3,           "France Ligue 3": 3,
  "Italy Serie C, Group A": 3,    "Italy Serie C, Group B": 3,
  "Italy Serie C, Group C": 3,    "Spain Primera Federacion": 3,

  /* The third wave, 25 Sep, bought with the paid API-Football month - see
     HARVEST_EXTRA. */
  "Northern Ireland Premiership": 1, "Northern Ireland Championship": 2,
  "Ecuador LigaPro Primera A": 1,   "Paraguay Division de Honor": 1,
  "Canada Canadian Premier League": 1, "Guatemala Liga Nacional": 1,
  "Japan J2 League": 2,
  "Ireland First Division": 2,       "Finland Ykkosliiga": 2,
  "Russia 1. Liga": 2,               "Egypt 2. Division A": 2,
  "Brazil Brasileiro Serie C": 3,    "Argentina Primera B": 3,
  "Denmark 2nd Division": 3,
};
...
function countryOfLeague(league) { return M.countryOf(league); }

/* The edge for home, positive when home is the higher division. Null when the
   two cannot be compared at all - a division missing from the ladder, or a
   country with no coefficient. Null means "do not predict this", never "treat
   as equal": silently calling a Premier League side and a third-tier side
   evens is the exact mistake this whole ladder exists to avoid.

   Within a country it is the difference of two rungs. Across a border it is
   the difference of two rungs PLUS the difference of two country handicaps,
   which is the only part that needed anything from outside our data. */
function tierEdge(homeLeague, awayLeague) {
  if (homeLeague === awayLeague) return 0;
  const h = rungOf(homeLeague), a = rungOf(awayLeague);
  if (h === null || a === null) return null;
  const hc = countryOfLeague(homeLeague), ac = countryOfLeague(awayLeague);
  if (hc === ac) return a - h;
  const hCountry = countryHandicap(hc), aCountry = countryHandicap(ac);
  if (hCountry === null || aCountry === null) return null;
  return (a + aCountry) - (h + hCountry);
}
/* Where a league sits inside its own country, in log goal-rate.
   TIER_HANDICAP only spells out the countries with a second division in our
   ratings. Everywhere else the only league we carry IS the top flight, so a
   known tier of 1 is a rung of zero - without this every European tie outside
   the big five would still be refused for want of a number that is zero by
   definition. Anything not in either map stays null and stays refused. */
function rungOf(league) {
  const explicit = TIER_HANDICAP[league];
  if (explicit !== undefined) return explicit;
  return LEAGUE_TIER[String(league || "")] === 1 ? 0 : null;
}
 ...
 */
/* WOMEN, WOMEN'S AND WOMENS. `women` does not match "Womens", so
   "USA National Womens Soccer League" walked through this gate - and
   scripts/namesaudit.js caught what happened next: Racing Louisville FC
   resolved to Racing Club of Argentina and San Diego Wave FC to San Diego FC
   of MLS. That fixture only stayed off the board because the tier check
   refused two clubs a continent apart, which is luck rather than a guard. */
/* "Simulated Reality League" and its kin are computer-generated matches the
   bookmakers list beside the real ones - eight SRL Nations League games on the
   card of 23 Sep 2026, named after real national teams. They were being
   dropped as "no league" by accident; they are refused here by rule, so no
   rating we hold can ever reach one. */
const UNRATED_COMPETITION =
  /\b(simulated|srl|esoccer|e-?football|virtual)\b|\b(amateur|amateure|junior(?:en|s)?|youth|jugend|academy|primavera|reserves?|jong|u ?1[5-9]|u ?2[0-3])\b|\bwomen(?:'s|s)?\b|\bfrauen\b|\bfemenin[ao]\b|\bf[ée]minin(?:e|es)?\b|\b(damallsvenskan|elitettan|kvindeliga|toppserien|liga f|nwsl)\b/i;
/* The last group is women's leagues whose names carry no "women" at all.
   "Sweden Damallsvenskan: Brommapojkarna v Malmo FF" was published on 25 Sep
   2026 priced off the two men's Allsvenskan sides. */

/* SENIOR COMPETITIONS WE HOLD NO RATINGS FOR.
 *
 * The regex above is about youth, reserve and women's football. This is the
 * other half of the same problem and it was reported the same way: a Russian
 * lower-division fixture reaching a slip. The team-name guard in model.js now
 * catches the reserve sides written "<club>-2 <city>", but a second-tier
 * fixture between two ordinary clubs has no marker to catch - it only stays
 * off the board for as long as neither name collides with a club we DO rate.
 * That is the fuzzy matcher behaving, not a guarantee.
 *
 * These are leagues, not cups, and the distinction matters. A cup tie between
 * rated clubs resolves correctly through leagueOfTeam and belongs on the board
 * - the EFL Cup, the Scottish League Cup, Copa Argentina and the FAI Cup are
 * all published and all correct. A league we hold no results for is different:
 * by definition its clubs are not clubs we rate, so any match is a collision.
 *
 * Also fixes a quieter wrong: relegated clubs keep their top-flight ratings,
 * so these fixtures were published at tier=1 - a second-division match dressed
 * as a first-division one. Measured on the live board: Hvidovre v Aalborg,
 * Yokohama FC v Albirex Niigata, Bryne v Stabaek, Arka Gdynia v LKS Lodz.
 *
 * An explicit list rather than a pattern, because "1. Liga" is Poland's SECOND
 * tier and Switzerland's THIRD while "1. Division" is Denmark's second - the
 * naming carries no reliable rank across languages, and a regex confident
 * enough to catch them would catch top flights too. It grows by observation;
 * add a name when the feed shows one. Delete a name if its results are ever
 * added to MAIN or EXTRA, since the exclusion only exists while ratings do not.
 */
const UNRATED_LEAGUES = new Set([
  /* National-team friendlies. The sides are rated - their friendlies are in
     the fit at half weight - but a coach trying six debutants is not the team
     the rating describes, so the fixture itself is not published. */
...
/* Russia's second tier is split into lettered divisions and numbered groups -
   "Russia 2. Liga, Division A", "Russia 2. Liga, Division B, Group 2" - so it
   is matched by prefix rather than listed six times. */
/* Wales's second tier, North and South: "Aberystwyth Town v Swansea" was
   published 25 Sep 2026 with that Swansea read as Swansea City. */
const UNRATED_LEAGUE_PREFIX = ["Russia 2. Liga", "Wales Cymru Championship"];

/* `idx` is optional and changes only the SENIOR-league half of the answer.
 *
 * Those entries are a statement about our data - "we hold no results for this
 * league" - and the comment above the list says to delete a name once results
 * arrive. Deleting by hand is the step that gets forgotten, and forgetting it
 * means a league we now rate stays refused with nothing anywhere saying why.
 * So when the index has actually been fitted on the league, the entry lets go
 * by itself. Warming leagues cross that line on their own - see HARVEST_EXTRA
 * in lib/liveresults.js.
 *
 * The youth, reserve and womens pattern is NOT gated on the index and must
 * never be: it is a rule about which football we will price at all, not a
 * report on what we happen to hold. */
function isUnratedCompetition(league, idx) {
  const s = String(league || "");
  if (UNRATED_COMPETITION.test(s)) return true;
  const rated = idx && idx.lIdx && idx.lIdx[canonLeague(s)] != null;
  if (rated) return false;
  if (UNRATED_LEAGUES.has(s.trim())) return true;
  for (const p of UNRATED_LEAGUE_PREFIX) if (s.trim().startsWith(p)) return true;
  return false;
}
 ...
 * request at all.
 *
 * ON A PAID PLAN THE ORACLE LEADS. Everything above is about 100 requests a
 * day. From 25 Sep 2026 the owner keeps API-Football Pro (7,500 a day), and it
 * is the one source that states a 90-minute score outright - the 10-minute
 * sweep already asks it first (lib/apifresults.js). `paid` comes from
 * ORACLE.planStatus() once per build, so if the plan ever lapses the next
 * build is back on the order above with no edit here. */
function scoreSources(matches, paid) {
  const fd = FD.fromMatches(matches);
  if (paid) {
    const list = [
      { name: "oracle", api: ORACLE, days: PAID_ORACLE_DAYS },
      { name: "soccervista", api: SV, days: SV_DAYS },
    ];
    if (fd.configured()) list.push({ name: "footballdata", api: fd, days: BACKFILL_DAYS });
    return list;
  }
...
/* The per-build budget bounds one build, not the day. Fifty deploys at three
   each still overrun a 100-a-day plan, and nothing stopped a build calling at
   zero remaining - how the first account was suspended. The plan reports what
   is left on every answer (budget.quota), so once it falls below this the
   build stops asking. A build learns the figure from its first call, so the
   floor leaves room for one call per build without ever reaching zero. */
const ORACLE_FLOOR = 10;
const oracleLow = (budget) => budget.quota != null && budget.quota < ORACLE_FLOOR;

/* The same limits on a paid plan (7,500 a day, any date). 40 a build at the
   worst deploy day seen (63) is 2,520, plus ~300 for the 10-minute sweep and
   the hourly stats fill - well inside the day, and ORACLE_FLOOR still stops a
   build short of zero. 14 days is how far back DB.recentResults fetches, so
   nothing older is ever held to ask about. Chosen per build by makeScoreBudget,
   never assumed: a lapsed plan gets the free numbers above. */
const PAID_ORACLE_BUDGET = 40;
const PAID_ORACLE_DAYS = 14;

function makeScoreBudget(paid) {
  return { spent: 0, memo: new Map(), quota: null,
    limit: paid ? PAID_ORACLE_BUDGET : ORACLE_BUDGET };
}
/* A budget made before `limit` existed, or by hand in a test, is a free one. */
const budgetLimit = (budget) => budget.limit || ORACLE_BUDGET;
...
function sourceCanAnswer(src, date, budget) {
  if (src.name === "oracle" && (budget.spent >= budgetLimit(budget) || oracleLow(budget))) return false;
  const t = Date.parse(date + "T00:00:00Z");
...
async function sourceRows(src, date, log, budget) {
  ...
  if (!sourceCanAnswer(src, date, budget)) {
    ...
    if (src.name === "oracle" && budget.spent >= budgetLimit(budget)) {
      const said = budget.said || (budget.said = new Set());
      if (!said.has("budget")) {
        said.add("budget");
        log.push(`oracle: not asked again this build (budget of ${budgetLimit(budget)} spent)`);
      }
    }
    if (src.name === "oracle" && oracleLow(budget)) {
      const said = budget.said || (budget.said = new Set());
      if (!said.has("floor")) {
        said.add("floor");
        log.push(`oracle: not asked again today (${budget.quota} left, floor ${ORACLE_FLOOR})`);
      }
    }
...
/* Why a fixture found nothing, in one line.
 *
 * "no source had these games" is true and useless. Lille v Paris SG sat held
 * back for days while football-data - which carries it, under exactly that
 * spelling, with a half-time score - was second in the chain. Nothing in the
 * output could say whether the source had been asked, had answered, or had
 * answered with a day nobody wanted.
 *
 * Every source here is already memoised for this date, so this costs no
 * request at all: it re-reads what the failed lookup already fetched. */
async function traceSources(date, log, budget, sources) {
  const parts = [];
  for (const src of (sources || SCORE_SOURCES)) {
    if (!src.api.configured()) { parts.push(`${src.name} off`); continue; }
    if (!sourceCanAnswer(src, date, budget)) { parts.push(`${src.name} skipped`); continue; }
    const rows = await sourceRows(src, date, log, budget);
    parts.push(`${src.name} ${rows ? rows.length : 0}`);
  }
  return parts.join(", ");
}
...
function logScoreBudget(budget, log) {
  ...
  log.push(`oracle: ${budget.spent} of ${budgetLimit(budget)} calls used, ` +
...
async function confirmScores(rows, log, budget, srcs) {
  ...
  for (const [date, group] of byDate) {
    ...
    for (const r of group) {
      ...
      if (!found) {
        /* Capped: a source outage would otherwise name every row it lost. */
        if (unmatched.length < 6) {
          unmatched.push(`${r.home} v ${r.away} [${await traceSources(date, log, budget, sources)}]`);
        }
        continue;
      }
      const m = found.m;
      ...
      /* Teams as well as the interval: "Win either half" names a side by name
         and the grader has no other way to tell which end of the fixture it
         was at. */
      const hit = GRADE.gradeLabel(r.tip, m.hg, m.ag,
        (m.hth != null && m.hta != null)
          ? { hth: m.hth, hta: m.hta, home: r.home, away: r.away } : null);
  ...
  if (unmatched.length) {
    log.push(`unmatched (rows each source held for the date): ${unmatched.join("; ")}`);
  }
  if (backfilled) {
...
function carryInPlay(fixtures, prevFixtures, now) {
  ...
  for (const p of (prevFixtures || [])) {
    ...
    const age = now - ko;
    /* A GAME PLAYED TODAY STAYS ON TODAY'S BOARD.
       The window used to end 4.5 hours after kick-off, which is long enough
       for the match and nothing else. Once it passed, a finished game left the
       board entirely - the feeds had already dropped it - and both locked
       cards lost the row they read back through fixtureById. The Pick and the
       Slip of the day then re-picked, mid-afternoon, onto whatever day still
       had games to come. Reported as "the potd and sotd changed mid day
       yesterday, and to games that won't play for another week".
       So a fixture is carried for the rest of the reader's day, settled or
       not. It rolls at Lagos midnight, which is when a day is actually over. */
    const lagosDay = (ms) => new Date(ms + LAGOS_OFFSET_MS).toISOString().slice(0, 10);
    const today = lagosDay(ko) === lagosDay(now);
    if (age < 0 || (age > IN_PLAY_MAX_MS && !today)) continue;
    /* Already carries a score AND is not today's, so it is settled history and
       the results path owns it from here. */
    if (p.hg != null && p.ag != null && !today) continue;
    ...
    /* A game carried because it is today's keeps the score it finished on -
       the cards above the board show a result rather than a confidence once
       the whistle has gone, and there is nowhere else for them to read it. */
    if (p.hg != null && p.ag != null) { row.hg = p.hg; row.ag = p.ag; }
    fixtures.push(row);
    carried++;
  }
  return carried;
...
/* How many tips each recent day published, for "how we did yesterday".

   ORDER MATTERS AND IS THE WHOLE FUNCTION. This build's board is counted FIRST,
   and the previous payload then fills in only the days that have left it.
   Carrying forward first and adding second would double every date still on the
   board, and with dozens of deploys a day that figure would run away inside an
   afternoon - reading as hundreds of ungraded tips on a forty-game card.

   Carried forward at all because the board holds today and onward: the moment a
   day rolls over its fixtures are gone, and with them any way to recount. So it
   is recorded, not recomputed - the same rule the pick of the day follows.

   Pruned to KEEP_DAYS so a map that is only ever added to cannot grow without
   bound, and a junk value from a hand-edited payload is dropped rather than
   published as a claim about how many games were on. */
const PUBLISHED_KEEP_DAYS = 10;
function mergePublished(prevPublished, fixtures, now) {
  const outByDate = {};
  for (const f of (fixtures || [])) {
    if (f && f.date && f.tip) {
      outByDate[f.date] = (outByDate[f.date] || 0) + 1;
    }
  }
  const prev = (prevPublished && typeof prevPublished === "object" &&
                !Array.isArray(prevPublished)) ? prevPublished : {};
  const cutoff = new Date((now || Date.now()) - PUBLISHED_KEEP_DAYS * 86400000)
    .toISOString().slice(0, 10);
  for (const d of Object.keys(prev)) {
    if (!/^\d{4}-\d{2}-\d{2}$/.test(d) || d < cutoff) continue;
    const n = Number(prev[d]);
    if (!Number.isFinite(n) || n <= 0) continue;
    /* THE HIGHEST THIS DAY EVER REACHED, NOT THE LAST TIME WE LOOKED.
       This used to let the current board win outright, on the reasoning that
       "the board is the authority while it holds the day". The board is not:
       it holds today and onward and DROPS each game as it kicks off, so its
       count for today falls all day long. Whatever it reads at the last build
       before midnight is what gets frozen and carried forward forever.
       Measured on the live payload, 4 Sep: 2026-09-03 was recorded as 11
       published while the results feed had already graded 19 of that day -
       fewer published than graded, which cannot happen. The consequence was
       the reader-facing one: yestCounts could no longer tell that games were
       missing, pendingByDate was empty, and "how we did yesterday" showed a
       partial day as though it were the whole card.
       Max is also what this field's own description asks for - how many tips
       were published that day, WHATEVER BECAME OF THEM. A postponed fixture
       was still a tip we published, so keeping it counted is right rather
       than merely convenient. The cost is that a day cannot shrink, so one
       inflated build sticks for KEEP_DAYS; that direction only ever says
       "some are still being graded" when they are not, which is the harmless
       way round compared with calling a half-graded day complete. */
    outByDate[d] = Math.max(Math.floor(n), outByDate[d] || 0);
  }
  return outByDate;
}
  ...
  for (const [date, group] of byDate) {
    for (const f of group) {
      ...
      const hit = GRADE.gradeLabel(f.tip, m.hg, m.ag,
        (m.hth != null && m.hta != null)
          ? { hth: m.hth, hta: m.hta, home: f.home, away: f.away } : null);
      if (hit === null) continue;   // a tip no final score can settle
      const model = {};
...
async function buildPayload(options = {}) {
  /* One budget for the whole build, shared by both grading passes, so a
     ...
     Sized by the plan API-Football says is active right now - /status is
     free - so the paid limits never outlive the plan. */
  const plan = await ORACLE.planStatus();
  const scoreBudget = makeScoreBudget(plan.paid);
  ...
  /* STOP ASKING A SERVER THAT IS TELLING US TO STOP.
   *
   * 84 requests go to football-data every build. That is nothing on a normal
   * day, and it is 84 requests into a 503 on a bad one - a site that answers
   * `Retry-After: 200` and gets asked another eighty times anyway. We have
   * form here: on 3 Sep 2026 a route that rebuilt the payload per request sent
   * them roughly 94,000 requests in a day, and their 503s began two days later.
   * Whether or not those two facts are connected, the polite build is the same
   * build.
   *
   * ONLY A 503 TRIPS THIS, never a 404. A missing file is normal - a season
   * file that has not been published yet 404s for a fortnight every July, and
   * treating that as an outage would stop the build fetching the files that DO
   * exist. A 503 is the server explicitly shedding load.
   *
   * Nothing is lost when it trips: every skipped file falls through to the
   * committed floor below, which is exactly what it is for. An outage build
   * costs them eight requests instead of eighty-four.
   *
   * Per-build only. A Vercel build keeps nothing between runs, so there is no
   * cooldown to carry - see makeBreaker in lib/upstream.js for the version
   * that does have one, in a process that lives long enough to need it. */
  const SHED_TRIP = 8;
  let shed = 0, shedTripped = false;
  const fetched = await pool(sources, cfg.concurrency, (s) => {
    if (shedTripped) return Promise.resolve({ ...s, error: "not asked - upstream shedding load" });
    return fetchText(s.url, cfg.fetchTimeoutMs).then((r) => {
      if (r.error === "HTTP 503" && ++shed >= SHED_TRIP) shedTripped = true;
      return { ...s, ...r };
    });
  });
  if (shedTripped) {
    log.push(`football-data answered 503 ${shed} times - stopped asking after ` +
      `${SHED_TRIP}, the floor covers the rest`);
  }

  /* Anything the network could not give us, take from the committed floor -
     see cachedResultsFor above for why finished seasons are safe to keep.
     Counted separately so the build log distinguishes "the source was down and
     we coped" from "everything was fine", which is the difference between a
     board to trust for weeks and one to fix today. */
  let fromCache = 0;
  for (const f of fetched) {
    if (!f.error) continue;
    const text = cachedResultsFor(f);
    if (!text) continue;
    f.text = text;
    f.cached = true;
    delete f.error;
  ...
  if (fromCache) {
    log.push(`served ${fromCache} result files from the committed floor - ` +
  ...
  log.push(off
    ? `country offsets: ${off.fitted} fitted from ${off.seasons.join(", ")} ` +
      ...
      `European home edge ${off.homeEdge}` +
      (off.pinned.length
        ? `; refusing ${off.pinned.join(", ")} - fitted onto the cap`
        : "")
  ...
  /* The harvested current season, added unconditionally rather than as a
     fallback. It is not a stand-in for a file that failed: while
     football-data is down these are the only recent results the fit has, and
     when football-data returns the same match arrives from both and the
     de-duplication below keeps one. */
  {
    const cutoff = new Date(cfg.minSinceDate);
    const live = loadLiveMatches().filter((m) => m.date && m.date >= cutoff);
    if (live.length) {
      let newest = null;
      for (const m of live) if (!newest || m.date > newest) newest = m.date;
      matches = matches.concat(live);
      log.push(`+${live.length} harvested results through ` +
        `${newest.toISOString().slice(0, 10)} (scripts/mkresults.js)`);
    }
  }

  let intlRows = [];
  /* NATIONAL TEAMS - see lib/internationals.js. Fitted SEPARATELY from the
     clubs, below: fitted jointly, 3,000 internationals among 43,000 club
     matches were crushed towards average - San Marino v Germany priced at
     1.37-1.93. Alone, the same file gives 0.51-7.33. Downloaded each build, one
     request to GitHub, with the committed copy under it for the same reason
     the results floor exists: a source we need only for history should never
     be able to take the board down. */
  {
    let text = null, src = "downloaded";
    const r = await fetchText(cfg.intlUrl || INTL.URL, cfg.fetchTimeoutMs);
    if (!r.error && r.text && r.text.length > 1000) text = r.text;
    if (!text) {
      try {
        text = zlib.gunzipSync(fs.readFileSync(path.join(__dirname, "..", "data", "internationals", "results.csv.gz"))).toString("utf8");
        src = `committed copy (${r.error || "short download"})`;
      } catch (e) { text = null; }
    }
    const intl = text ? INTL.parse(text, INTL.SINCE) : [];
    if (intl.length) {
      intlRows = intl;
      const played = intl.filter((m) => !m.neutral).length + intl.filter((m) => m.neutral).length / 2;
      const newest = intl.reduce((a, m) => (m.date > a ? m.date : a), intl[0].date);
      log.push(`internationals: ${played} matches through ` +
        `${newest.toISOString().slice(0, 10)}, ${src}`);
    } else {
      log.push("internationals: none loaded - national teams stay unrated today");
    }
  }
  ...
  if (extraGot === 0 && Object.keys(cfg.extra).length && !shedTripped) {
  ...
  /* HOW OLD IS THE FORM THIS BOARD IS FITTED ON?
   *
   * Not a curiosity. While football-data is unreachable the committed floor
   * carries the model, and the floor holds only what it held when it was
   * committed - measured 6 Sep 2026, nine divisions had any of the current
   * season at all, their newest result between six and thirty-six days old,
   * and every other division was fitted on last season.
   *
   * The board looks completely normal while that is true, which is the whole
   * problem. prebuild's fallback comment warns that a site quietly serving an
   * old board for days is the failure it sits one step from; keeping the site
   * up through an outage was the first half, saying how fresh what it serves
   * is, is the second.
   *
   * Taken from the matches actually FITTED - after de-duplication and after
   * thin leagues are dropped - not from what was downloaded, so it cannot
   * claim freshness the model never got. */
  let formThroughMs = null;
  for (const m of matches) {
    const t = (m.date instanceof Date) ? m.date.getTime() : Date.parse(m.date);
    if (!Number.isFinite(t)) continue;
    if (formThroughMs === null || t > formThroughMs) formThroughMs = t;
  }
  const formThrough = (formThroughMs === null) ? null
    : new Date(formThroughMs).toISOString().slice(0, 10);
  const formStaleDays = (formThroughMs === null) ? null
    : Math.max(0, Math.floor((Date.now() - formThroughMs) / 86400000));
  log.push(`form fitted through ${formThrough || "unknown"}` +
    (formStaleDays === null ? "" : ` (${formStaleDays}d old)`) +
    (fromCache ? ` - ${fromCache} of those files came from the committed floor` : ""));
  ...
  cfg._sources = scoreSources(matches, plan.paid);
  log.push(`score sources: ${cfg._sources.map((x) => x.name).join(" -> ")}` +
    (plan.paid ? ` (API-Football paid, ${plan.limit}/day)` : ""));

  // fixtures: both feeds
  /* Skipped entirely while they are shedding load. These two are league-only
     and SportyBet carries the large majority of the card including every cup
     tie, so the cost of not asking is small and the cost of asking is two more
     requests at a server that has said no eight times already. */
  const fxUrls = shedTripped ? []
    : [`${BASE}/fixtures.csv`, `${BASE}/new_league_fixtures.csv`];
  if (shedTripped) log.push("fixtures: not asked - upstream shedding load");
    ...
    if (!sbRes.error) {
      ...
      for (const m of (j.matches || [])) {
        ...
        fixtures.push({
          ...
          /* WHOSE CARD THIS ROW CAME FROM, which decides a clash. See the
             kickoff guard below: when two feeds describe one match with
             different names, the bookmaker's spelling is the one the odds
             pair against and the one a reader can book. */
          src: "sporty",
  ...
  /* National sides on their own terms: a long memory and a light pull to the
     mean, because they play ten times a year and change slowly. Walked forward
     over twelve months of competitive internationals: 1X2 logloss 0.789 at
     these settings against 0.835 at the clubs'. No shots blend - the file has
     no shots. */
  const intlModel = intlRows.length
    ? M.fitModel(intlRows, { halfLife: INTL.HALF_LIFE, reg: INTL.REG, xgWeight: 0 })
    : null;
  log.push(`fitted in ${Date.now() - t0}ms, k=${model.k.toFixed(1)}, ` +
           `home adv ${model.hadv.toFixed(3)}`);

  /* CORNERS, off the same matches, for the leagues whose file carries them.
     Separate from the goals fit because it answers null for most of the board
     and nothing above it may assume otherwise - see lib/corners.js. */
  const corners = CORN.fitCorners(matches);
  log.push(corners
    ? `corners fitted over ${corners.matches} matches in ${corners.league.size} ` +
      `leagues, mean ${corners.mean.toFixed(2)}, k=${corners.k.toFixed(1)}`
    : "corners: no league carried the column, nothing priced");

  /* SHOTS, THE SAME FIT ASKED ABOUT A DIFFERENT COLUMN. Total shots and shots
     on target sit in the same main-division rows as the corners, so this is
     fitCorners pointed at `hs`/`as` and `hst`/`ast` - one implementation, not
     two that could drift apart on shrinkage or dispersion.
     Measured before it went in, walked forward a week at a time over 26 weeks
     against a league-average baseline: total shots 1.8-4.5% better logloss
     across lines 19.5-27.5, on target 1.0-1.7% - both clearer than the
     corners' 0.9%, because how often a side shoots is more its own habit than
     how many corners it wins. It also runs 3-5 points LOW on the over side:
     home shot volume rose faster than two seasons of history tracks (2.
     Bundesliga 17% over its prediction, Bundesliga 13%). That is published as
     a known bias, not corrected with a factor fitted to one season.
     Half-life 120, not the corners' 260: at the 23.5 line it scored 0.6414
     against 0.6454, and 60 was no better. Shots on target use the same without
     having been swept separately. */
  const SHOT_OPTS = { home: "hs", away: "as", minMean: 5, maxMean: 25, halfLife: 120 };
  const SOT_OPTS = { home: "hst", away: "ast", minMean: 1.5, maxMean: 9, halfLife: 120 };
  const shots = CORN.fitCorners(matches, SHOT_OPTS);
  const sot = CORN.fitCorners(matches, SOT_OPTS);
  log.push(shots
    ? `shots fitted over ${shots.matches} matches in ${shots.league.size} leagues, ` +
      `mean ${shots.mean.toFixed(2)}, k=${shots.k.toFixed(1)}; on target ` +
      (sot ? `mean ${sot.mean.toFixed(2)}, k=${sot.k.toFixed(1)}` : "not fitted")
    : "shots: no league carried the column, nothing priced");
  ...
  /* EACH LEAGUE'S OWN DRAW RATE, as a multiple of the average across leagues.
     Derived from the results already downloaded rather than written down,
     which is the same rule "high scoring" follows: a list of league names
     needs maintaining and goes stale the season a division changes character,
     while a rate follows whatever is actually happening.
     Measured over 70,608 matches on 3 Sep 2026, the spread is real and worth
     having: Argentina 30.2% against a 26.4% average - the second-highest rate
     on the board, on the largest sample near the top, and the lowest-scoring
     league of the 36 at 2.22 goals a game. France Ligue 1 sits at 22.9%.
     Keyed on idx.leagues[...], which is the namespace matches carry and the
     model is queried with - a key from anywhere else resolves to nothing and
     the term silently does no work. */
  const DRAW_MIN_N = 150;
  const drawLift = (() => {
    const acc = new Map();
    for (const m of matches) {
      const e = acc.get(m.league) || { n: 0, d: 0 };
      e.n++; if (m.hg === m.ag) e.d++;
      acc.set(m.league, e);
    }
    let tn = 0, td = 0;
    for (const v of acc.values()) if (v.n >= DRAW_MIN_N) { tn += v.n; td += v.d; }
    const base = tn ? td / tn : 0.264;
    const out = new Map();
    for (const [name, v] of acc) {
      if (v.n >= DRAW_MIN_N) out.set(name, (v.d / v.n) / base);
    }
    return out;
  })();
  ...
  const dropped = [];
  const seenKey = {};
  /* NO CLUB PLAYS TWO MATCHES AT ONCE, and on 21 Sep the board published two
     that did: "Barracas Central v Ind. Rivadavia" from the bookmaker's card
     and "Barracas Central v Independiente" from the league feed, same league,
     same 22:00 kick-off. Both are real clubs, so the name matcher resolved
     each away side to a different team and the dedupe below - keyed on the
     two matched teams - saw two different fixtures. It is the SHORTER name
     that was wrong: a feed wrote "Independiente" for Independiente Rivadavia.
     The cost was not cosmetic. Both rows paired to the SAME SportyBet event,
     so the day's own booking code went out with five legs and four games -
     and the books that refuse a same-game multiple, BetKing and Betpawa,
     rejected a slip whose fixtures all looked different.
     So a second fixture sharing a club AND a kick-off instant is dropped
     here. The bookmaker's row wins because it is the card the site prices and
     books against; with no bookmaker row the first one seen wins, which is
     the behaviour this loop already had. */
  const bySlot = {};
  fixtures = fixtures.slice().sort(
    (a, b) => (b.src === "sporty" ? 1 : 0) - (a.src === "sporty" ? 1 : 0));
  for (const f of fixtures) {
    ...
    if (isUnratedCompetition(f.league, idx)) {
      unmatched++;
    ...
    let edge = 0, crossTier = 0, crossCountry = 0;
    if (li == null) {
      ...
      if (hl != null && al != null) {
        ...
        /* A border is not a division. The gap between two countries is
           anchored on a published coefficient rather than measured here, so
           these ties are marked separately and treated more carefully than a
           domestic cup tie - goals only, and shrunk harder. */
        if (countryOfLeague(idx.leagues[hl]) !== countryOfLeague(idx.leagues[al])) {
          crossCountry = 1;
          /* AND THESE TIES CARRY MORE HOME ADVANTAGE THAN THE DOMESTIC MODEL
             KNOWS. Measured over 618 cross-border matches: home sides beat
             expectation by 7.0%, away sides fell 4.8% short, and the country
             offsets could not absorb it - an offset flips sign when the pair
             swap venues and this does not. It is fitted alongside them and
             added here, where it reaches predictTotals as `+v` home and `-v`
             away, the same shape as the edge itself. Zero without an
             artefact, so the fallback is untouched. */
          edge += EUROFF.homeEdge();
        }
      }
    }
    /* A NATIONAL-TEAM FIXTURE goes to the international fit instead - its own
       index, so no club can ever resolve onto a country and no country onto
       a club. Only when the club side found nothing, and only for a senior
       men's national competition. */
    let mdl = model, isIntl = false;
    if (li == null && intlModel && INTL.isNationalCompetition(f.league) &&
        intlModel.index.tIdx[f.home] !== undefined &&
        intlModel.index.tIdx[f.away] !== undefined) {
      isIntl = true; mdl = intlModel;
    }
    if (li == null && !isIntl) { unmatched++; dropped.push(`${f.home} v ${f.away} (no league)`); continue; }
    const lgName = isIntl ? INTL.LEAGUE : idx.leagues[li];
    ...
    const h = isIntl ? f.home
      : crossTier ? M.matchTeam(idx, f.home, leagueOfTeam(idx, f.home))
                  : M.matchTeam(idx, f.home, li);
    const a = isIntl ? f.away
      : crossTier ? M.matchTeam(idx, f.away, leagueOfTeam(idx, f.away))
    ...
    const hLeague = isIntl ? lgName : idx.leagues[crossTier ? leagueOfTeam(idx, f.home) : li];
    const aLeague = isIntl ? lgName : idx.leagues[crossTier ? leagueOfTeam(idx, f.away) : li];
    const tier = fixtureTier(hLeague, aLeague);

    const p = M.predictTotals(mdl, h, a, lgName, edge);
    ...
    seenKey[dedup] = 1;
    const k = M.markets(p, { fhShare: fhShare[lgName], k: mdl.k });
    const tip = M.bestTip(k, { crossTier: !!crossTier, crossCountry: !!crossCountry });
    /* bestTip returns null on a cross-border tie it has no safe goals call
       for. That is the deal: we speak on these only when the goals market is
       strong enough to carry the weight, and stay quiet otherwise rather than
       fall back to a winner we cannot stand behind. */
    if (!tip) { unmatched++; dropped.push(`${f.home} v ${f.away} (cross-border, no safe goals call)`); continue; }
    // uncertainty band: thin recent data widens confidence toward 50/50
    const sup = M.support(mdl, h, a);
    ...
    if (crossCountry) shrink = Math.min(shrink, CROSS_COUNTRY_SHRINK);
    ...
    /* The calibration is measured on graded CLUB tips. Applied to national
       teams it pushed a raw 81% Over 1.5 - which is what top-v-top
       internationals actually do, 81.3% over 64 games - to 92%. */
    const tipPcal = isIntl ? tip.p : calibrator.apply(tip.label, tip.p);
    const tipPadj = 0.5 + (tipPcal - 0.5) * shrink;
    /* THREE DECIMALS, NOT FOUR. Every probability on every fixture ships in
       predictions.json, which every visitor downloads, and the fourth digit
       was 20 KB of it on the wire (brotli) at 96 leagues - the page stood at
       689 of its 700 KB budget on 25 Sep. 0.1% is finer than anything the
       page prints and far finer than the model knows; a banker threshold can
       move by at most 0.05%, which is noise next to the calibration itself. */
    ...
    if (!label) {
      ...
      const country = M.countryOf(lgName);
      label = country ? `${country} Cup` : "Cup";
    }

    /* After `label`, not before it: the league this fixture is finally called
       is what decides which league mean its corners are scaled by, and a cup
       tie is not called anything until the lines above have run. */
    const cn = CORN.cornersFor(corners, h, a, label);
    const sn = CORN.cornersFor(shots, h, a, label);
    const tn = CORN.cornersFor(sot, h, a, label);

    /* ONE CLUB, ONE KICK-OFF - and down here, after `kickoff` exists, which is
       the whole reason the first attempt at this guard fired on nothing. The
       two feeds do not agree on what `f.date` carries: SportyBet's is the
       instant, football-data's is the day with the clock in `f.time`, so the
       same 22:00 match arrived as two different slots and every comparison
       passed. Keyed on the emitted kickoff, which is the one field both have
       been normalised into. */
    const clash = slotClash(bySlot, kickoff || f.date.toISOString(), [h, a],
                            `${h} v ${a}`);
    if (clash) {
      unmatched++;
      dropped.push(`${h} v ${a} (clashes with ${clash})`);
      continue;
    }
    ...
    out.push({
      ...
      /* A national-team fixture, priced by the international fit. Kept off the
         Pick and the Slip of the day while that fit has no graded record of
         its own - see choosePotd and renderSlipOfDay. */
      intl: isIntl ? 1 : undefined,
      cross_country: crossCountry ? 1 : undefined,
      ...
      /* A result crossed with goals. Both bookmakers sell all six as one
         market, so a leg built from these books like any other - see
         PASSTHROUGH_MAP in the API for the ids. Baked whether or not a chip is
         switched on today: the payload is what the record grades, and a market
         cannot earn its record without one. */
      home_btts: r(k.homeOrBtts),
      away_btts: r(k.awayOrBtts),
      home_o25: r(k.homeOrO25),
      away_o25: r(k.awayOrO25),
      /* Bet9ja only, and priced anyway - see drawOrO15 in lib/model.js. The
         whole 1.5 rung of this family is theirs: draw, home and away. */
      draw_o15: r(k.drawOrO15),
      home_o15: r(k.homeOrO15),
      away_o15: r(k.awayOrO15),
      /* Win either half. Not readable off the full-time matrix - 2-1 says
         nothing about the interval - so it comes from two half distributions
         combined as independent, and the published record is what says whether
         that assumption holds: 945 graded, 60.0% against 58.9% predicted. */
      h_win_half: r(k.homeWinHalf),
      a_win_half: r(k.awayWinHalf),
      /* EXPECTED CORNERS, TWO NUMBERS RATHER THAN FOURTEEN PROBABILITIES.
         Both books sell seven total lines and twenty per-side ones, and every
         one of them is a tail of the same distribution - so the fixture
         carries the two rates and the page does the arithmetic. A line neither
         book quotes today is answerable tomorrow without a rebuild, and the
         payload grows by two numbers instead of thirty-four.
         Absent, not zero, on the leagues whose file has no corners column. */
      ch: cn ? r2(cn.ch) : undefined,
      ca: cn ? r2(cn.ca) : undefined,
      /* Expected shots and shots on target per side, on the corners' terms:
         two rates each, tails computed where they are read, absent where the
         league's file carries no shots column. */
      sh: sn ? r2(sn.ch) : undefined,
      sa: sn ? r2(sn.ca) : undefined,
      th: tn ? r2(tn.ch) : undefined,
      ta: tn ? r2(tn.ca) : undefined,
      /* WHERE A DRAW IS ACTUALLY LIVE, and it is not where this used to look.
         It required k.draw >= 0.30, which was measured on 3 Sep 2026 over 863
         held-out matches to fire on TWO of them - 0.2% of the card, against
         the "roughly one game in thirty" its own comment claimed. A flag that
         never fires is indistinguishable from one that is switched off.
         What does predict a draw is the two sides being evenly matched, and it
         is a cliff rather than a slope: within 0.10 expected goals of each
         other the draw rate is 33.3% against a 24.2% base, a 1.36x lift; past
         0.15 it collapses to about 21% and stays flat wherever else you look.
         Expected GOALS, tested alongside it, is nearly worthless on its own -
         1.05x - and stacking it on top of evenness made things worse, so it is
         not here.
         The league is, because it earns it separately: a draw-heavy division
         gets a wider band rather than a different rule. 0.12 is inside the
         cliff; 0.18 is the widening, and still short of where the effect dies. */
      draw_watch: (function(){
        const gap = Math.abs(k.lh - k.la);
        const lift = drawLift.get(lgName) || 1;
        return gap < (lift >= 1.10 ? 0.18 : 0.12);
      })(),
  ...
  try {
    ...
    if (got.ok && got.rows.length) {
      ...
      let added = 0, held = 0;
      /* Named, not just counted. "2 recorded result(s) held back" says
         something is stuck and nothing about whether it is fixable - a source
         that could be added, or a cup tie no feed carries. The names are the
         difference between a number to worry about and a decision. Capped, so
         a bad day cannot bury the build output. */
      const heldNames = [];
      ...
      for (const r of pending) {
        ...
        if (!v) {
          ...
          if (heldNames.length < 12) {
            heldNames.push(`${r.match_date} ${r.home} v ${r.away}` +
              (r.league ? ` (${r.league})` : ""));
          }
          continue;
        }
        /* Spread first, identity second - see the same note in
           scripts/prebuild.js. A model key named `home` would otherwise
           replace the club with a probability. */
        results.push(Object.assign({},
          (r.model && typeof r.model === "object") ? r.model : {},
          { date: r.match_date, league: r.league || "",
      ...
      if (held) {
        log.push(`${held} recorded result(s) held back - no confirmed final score`);
        log.push(`held back: ${heldNames.join("; ")}` +
          (held > heldNames.length ? ` and ${held - heldNames.length} more` : ""));
      }
      /* CORNERS AND SHOTS onto EVERY result the page gets, whichever path
         brought its score - lib/statsfill.js writes them to the stored row
         after full time, and the page settles corners and shots legs on them.
         Only real counts travel: -1 is "API-Football had none". */
      const statsByKey = new Map();
      for (const r of got.rows) {
        if (Number.isFinite(r.hc) && r.hc >= 0 && Number.isFinite(r.ac) && r.ac >= 0) {
          statsByKey.set(KEY.fixtureKey(r.match_date, r.home, r.away), r);
        }
      }
      let withStats = 0;
      for (const r of results) {
        const s = statsByKey.get(KEY.fixtureKey(r.date, r.home, r.away));
        if (!s) continue;
        r.hc = s.hc; r.ac = s.ac;
        if (Number.isFinite(s.hsh) && s.hsh >= 0 && Number.isFinite(s.ash) && s.ash >= 0) { r.hsh = s.hsh; r.ash = s.ash; }
        withStats++;
      }
      if (statsByKey.size) log.push(`corners/shots on ${withStats} result(s)`);
  ...
  async function enrichWithFTScores() {
    ...
    for (const d of [0, 1]) {
      ...
      /* Same order as confirmScores, and for the same reason: whichever
         source answers. Through sourceRows and the build's one budget: this
         loop used to call resultsFor directly, uncapped and unmemoised, so it
         paid the oracle again for dates confirmScores had already fetched.
         The build's own list, not the module default, or a paid plan would
         put the oracle first there and second here. */
      for (const src of cfg._sources) {
        const rows = await sourceRows(src, date, log, scoreBudget);
        if (rows && rows.length) { oracleRows.set(date, rows); break; }
      }
    }
  ...
  /* Count what THIS build is publishing, then carry the older days forward.
     Today's entry is rewritten on every build, which is right: the latest board
     is what a reader saw. Older entries are never recomputed, only kept, and
     the map is pruned so it cannot grow without bound. */
  const publishedByDate = mergePublished(cfg.prevPublished, out);
  ...
  return {
    ...
    /* generatedAt says when the BOARD was made. These say how old the RESULTS
       behind it are, and during a source outage the two say very different
       things - a board built ten minutes ago on form that stops three weeks
       back. `degradedSources` is how many result files the network could not
       give us, so a reader-facing warning can distinguish "quiet week" from
       "the feed is down". */
    formThrough: formThrough,
    formStaleDays: formStaleDays,
    degradedSources: fromCache || 0,
    matches: matches.length,
    /* The shared spread behind every corners line. One number for the whole
       board because it is a property of the sport, not of a fixture - see
       lib/corners.js. Absent when nothing was fitted, which is the page's cue
       that corners cannot be priced at all today. */
    cornersK: corners ? Math.round(corners.k * 100) / 100 : undefined,
    shotsK: shots ? Math.round(shots.k * 100) / 100 : undefined,
    sotK: sot ? Math.round(sot.k * 100) / 100 : undefined,
    leagues: [...new Set(out.map((f) => f.league))].sort(),
    fixtures: out,
    potd: potd,
    results: results,
    /* The fixtures the build could not price used to ride here as `unmatched`.
       Nothing reads that field - not the page, not a script - and at 96 leagues
       it was 45 KB of every visitor's download (12 KB brotli), which is what
       pushed the first load past its 700 KB budget on 25 Sep. The build log
       still names the first 25 ("dropped: ..."), which is where a person
       looks. */
    record: record,
    ...
    /* {date: n} - how many tips were PUBLISHED on each recent day, whatever
       became of them.
       pendingByDate above only counts rows the record already holds and cannot
       confirm. It says nothing about a tip that was never filed at all, which
       is the common case: a tip reaches the record only while its fixture is
       still on the board, so a late game with no build before midnight is
       simply never written down. On 2 Sep the board carried 40 fixtures and 31
       were graded; pendingByDate was empty, so "how we did yesterday" showed
       18 of 31 at 58% with nothing saying nine were still out.
       Carried forward from the previous payload, because the board holds today
       and onward - the moment a day rolls over there is nothing left to count.
       Anything that must stay put has to be recorded, not recomputed. */
...
function choosePotd(fixtures, prev, nowMs) {
  ...
  const all = fixtures || [];
  /* THE PICK BELONGS TO THE READER'S DAY (24 Sep 2026). It changed twice in
     one morning: at 01:53 Lagos the only club game dated the 24th was Seattle
     v Real Salt Lake at 02:30 Lagos, so it was picked; once it kicked off the
     day had "nothing left to come" and the pick jumped to Inverness tomorrow;
     then the day's real card (Caen v Rouen, 19:45) reached the feeds and the
     pick jumped back. The rule is now the Lagos day, not the fixture's UTC date
     and not "is anything still to come":
       - a pick whose game falls inside today in Lagos, from 06:00, stays until
         Lagos midnight - kicked off, played, whatever - showing its result;
       - a pick in the small hours (before 06:00 Lagos) is only provisional: it
         stands until a daytime game for today is on the board, then gives way.
     A small-hours game also ranks last when choosing fresh - see `night`. */
  const lagosDay = (ms) => new Date(ms + LAGOS_OFFSET_MS).toISOString().slice(0, 10);
  const kickMs = (f) => (f && f.kickoff ? Date.parse(f.kickoff) : NaN);
  const lday = (f) => (isFinite(kickMs(f)) ? lagosDay(kickMs(f)) : f.date);
  const night = (f) => isFinite(kickMs(f)) &&
    ((kickMs(f) + LAGOS_OFFSET_MS) % 86400000) < 6 * 3600000;
  const today = lagosDay(now);
  if (prev && prev.id) {
    const pf = all.find((f) => potdFid(f) === prev.id);
    if (pf && lday(pf) === today) {
      const daytimeToCome = all.some((f) => lday(f) === today && !f.intl && !night(f) &&
        isFinite(kickMs(f)) && kickMs(f) > now);
      if (!night(pf) || !daytimeToCome) return prev;
      prev = null;             /* a small-hours pick, and the real day has arrived */
    }
  }
  /* National teams only when there is nothing else on the board at all. The
     demerit below reorders within a day, and that was not enough: on 23 Sep
     2026 the rest of the day was internationals, so Bahamas v Saint-Martin -
     90%, on 21 matches of Saint-Martin history - still took the headline. A
     club game tomorrow, wearing its "Tomorrow" chip, is the better call. */
  const clubs = (fixtures || []).filter((f) => !f.intl);
  if (clubs.length) fixtures = clubs;
  ...
  /* PAST THE READER'S MIDNIGHT IS NOT TODAY.
     `f.date` is the UTC calendar date of the kickoff and Lagos is UTC+1, so
     anything kicking off from 23:00Z onward carries today's date while landing
     after the reader's midnight. Reported as "why is pick of the day giving me
     a game past midnight": an MLS game at 23:45Z, headlined all day as today's
     call, kicking off at 00:45 tomorrow.
     The same fixture was half-fixed once already - `started` takes it out the
     morning AFTER, once it has kicked off. This is the other half, the whole
     day before.
     A demerit rather than a filter, and it sorts ahead of confidence for the
     same reason the tier cost does: on a night whose card is nothing but
     overnight games the headline still has to say something, and one of them
     is the honest answer. */
  const dayEndsMs = Date.parse(day + "T00:00:00.000Z") + 86400000 - LAGOS_OFFSET_MS;
  const overnight = (f) => !!(f.kickoff && Date.parse(f.kickoff) >= dayEndsMs);

  /* The carry-forward keeps a pick across rebakes, which is the point of it -
     but not when the pick it is keeping is one this rule would no longer make.
     Without this the 06:30 rebake re-affirms yesterday's overnight choice
     every day and the fix never reaches the page. */
  if (prev && prev.date === day && prev.id &&
      onDay.some((f) => potdFid(f) === prev.id) &&
      !(onDay.some((f) => potdFid(f) === prev.id && overnight(f)) &&
        onDay.some((f) => !overnight(f) && !started(f)))) return prev;
  ...
  /* A national-team fixture sorts behind every club game on the day. Not a
     ban: on a day of nothing but internationals one of them still has to lead
     the page. But a minnow tie throws off 90%+ numbers - Bahamas v
     Saint-Martin took the pick on 23 Sep 2026, on 21 matches of history for
     Saint-Martin, almost all against other minnows - and the headline is the
     last place for the least-evidenced number on the board. */
  const demerit = (f) => (f.thin ? 1 : 0) + (overnight(f) || night(f) ? 2 : 0) + (f.intl ? 4 : 0);
...
module.exports = { seasonCodes, buildPayload, buildSourceList, rowsToMatches, fetchText, slotClash, choosePotd, POTD_TIER_COST, potdScore, leanResults, scoreForTip, RESULT_MODEL_KEYS, MAIN, EXTRA, EXTRA_FILE, DEFAULTS, recordPublishedTips,
  carryInPlay, IN_PLAY_MAX_MS, scoreSources,
  mergePublished, PUBLISHED_KEEP_DAYS,
  firstScoreSource, findScore, sourceRows,
  makeScoreBudget, logScoreBudget, sourceCanAnswer, applyStatsOverlay,
  ORACLE_BUDGET, ORACLE_MAX_AGE_DAYS, PAID_ORACLE_BUDGET, PAID_ORACLE_DAYS,
  confirmScores,
