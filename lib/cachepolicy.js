...
/* Worst-case age a reader can be served, given that the two caches lapse
   ...
   FRESH ONLY. It deliberately excludes stale-while-revalidate - see
   worstServedAge below for why that distinction is not academic. */
function worstCaseStaleness(policy) {
  const p = policy || {};
  return windowOf(p.cdn) + windowOf(p.vercel);
}

/* The same question asked honestly: how old can the body actually be?
 *
 * `stale-while-revalidate` is not a hint. A hit inside the grace window is
 * served the OLD body and the refresh happens behind the reader, so the age a
 * reader can see is s-maxage PLUS the grace, on each cache, summed.
 *
 * This exists because the distinction cost us a day. /predictions.json shipped
 * `s-maxage=3600, stale-while-revalidate=86400`; windowOf scored that 3600 and
 * every staleness test was happy, while Vercel's edge served a board generated
 * the previous day - the Champions League fixtures were missing from the site
 * for hours after the deploy that added them (7 Sep 2026).
 *
 * Both functions are correct for their own question, so neither replaces the
 * other: windowOf answers "how long until a cache revalidates", this answers
 * "how stale can the reader's copy be". Reach for this one when the budget is
 * about what a person sees. */
function graceOf(cacheControl) {
  if (!cacheControl || /no-store/.test(cacheControl)) return 0;
  const m = /stale-while-revalidate=(\d+)/.exec(cacheControl);
  return m ? Number(m[1]) : 0;
}

function servedAgeOf(cacheControl) {
  return windowOf(cacheControl) + graceOf(cacheControl);
}

function worstServedAge(policy) {
  const p = policy || {};
  return servedAgeOf(p.cdn) + servedAgeOf(p.vercel);
}
