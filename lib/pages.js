...
/* THE BOOKMAKERS' OWN NAMES, THE WAY THE APP WRITES THEM. index.html sets
   ...
   file. */
:root{--b9-red:#D42127;--b9-green:#14B151;--bk-gold:#FFC400}
@media (prefers-color-scheme:light){:root{--bk-gold:#A97400}}
.sbm{color:var(--brand);font-weight:800;white-space:nowrap}
.b9m,.bkm{font-weight:800;white-space:nowrap}
...
function pageFooter() {
  return `<footer>
    ...
    <div class="foot-cols">
      <div><h4>What this is</h4><p>Statistical football predictions across 38 leagues, rebuilt every morning from results.</p></div>
...
/* The archive's heading, with the gold keyline the rest of the site uses to
   mark a section that matters. The count on the right is the claim in one
   number: how many days are on the record. */
.ck-arch{display:flex;align-items:baseline;gap:12px;margin:30px 0 4px;
  padding-left:11px;border-left:3px solid var(--accent);color:var(--text);
  font-size:14px;letter-spacing:.06em}
.ck-arch span{margin-left:auto;color:var(--faint);font-size:11.5px;
...
function renderCodesHub(entries, resultOf) {
  ...
  const top = latest
    ? '<section class="ck-hero">' +
      ...
      " games, one slip, " +
      (slipOdds(latest) ? "<em>" + oddsMark(slipOdds(latest)) + "</em> odds." : "free to load.") +
      "</h2>" +
      codeTickets(latest) +
      oddsPanel(latest) +
      (latest.firstKickoff
  ...
  return staticPage({
    ...
    h1html: 'Free <em>booking codes</em>',
