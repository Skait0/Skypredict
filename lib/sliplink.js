...
/* `{H}` and `{A}` are filled with this leg's own teams when it renders - see
   legRows. A combination market is one side OR something, and "Home or over
   1.5" on a card that already names the teams reads as a different game. */
const MARKETS = {
  ...
  /* THE SEVEN MARKETS ADDED ON 14 SEP, WHICH THIS TABLE DID NOT LEARN.
     A slip carrying one of them could not travel at all: decode refuses a code
     it does not know, so POST /api/share was rejected, no short link was
     stored, the Share button fell back to the long URL - and that long URL
     opened on "that slip has a bet we do not offer". The enormous link and the
     dead link were the same missing row. Every code either builder can put on
     a slip belongs here, and test/sliplink.test.js now pins that against the
     chip table in the page. */
  "MIX_1_OV_1.5": "{H} or over 1.5 goals", "MIX_2_OV_1.5": "{A} or over 1.5 goals",
  "MIX_X_OV_1.5": "Draw or over 1.5 goals",
  "MIX_1_OV_2.5": "{H} or over 2.5 goals", "MIX_2_OV_2.5": "{A} or over 2.5 goals",
  "MIX_X_OV_2.5": "Draw or over 2.5 goals",
  "MIXGG_1": "{H} or both teams to score", "MIXGG_2": "{A} or both teams to score",
  "MIXGG_X": "Draw or both teams to score",
  "WINHALF_H_Y": "{H} to win a half", "WINHALF_A_Y": "{A} to win a half",
  /* The corners chip, 23 Sep - the same lesson as the seven above, learned
     before shipping this time: test/marketwiring caught it. */
  "CORNERS_OV_7.5": "Over 7.5 corners", "CORNERS_UN_7.5": "Under 7.5 corners",
  "CORNERS_OV_8.5": "Over 8.5 corners", "CORNERS_UN_8.5": "Under 8.5 corners",
  "CORNERS_OV_9.5": "Over 9.5 corners", "CORNERS_UN_9.5": "Under 9.5 corners",
  "CORNERS_OV_10.5": "Over 10.5 corners", "CORNERS_UN_10.5": "Under 10.5 corners",
  /* Corners per team, 25 Sep - see TEAM_CORNER_CODES in the page. The page
     says "{H} over 3.5 corners" (mLabel), and so does the card. */
  "CORNERS_H_OV_3.5": "{H} over 3.5 corners", "CORNERS_H_OV_4.5": "{H} over 4.5 corners",
  "CORNERS_H_OV_5.5": "{H} over 5.5 corners", "CORNERS_A_OV_2.5": "{A} over 2.5 corners",
  "CORNERS_A_OV_3.5": "{A} over 3.5 corners", "CORNERS_A_OV_4.5": "{A} over 4.5 corners",
  /* Total shots, overs only - see SHOTS_CODES in the page. */
  "SHOTS_OV_19.5": "Over 19.5 shots",
  "SHOTS_OV_20.5": "Over 20.5 shots",
  "SHOTS_OV_21.5": "Over 21.5 shots",
  "SHOTS_OV_22.5": "Over 22.5 shots",
  "SHOTS_OV_23.5": "Over 23.5 shots",
  "SHOTS_OV_24.5": "Over 24.5 shots",
  "SHOTS_OV_25.5": "Over 25.5 shots",
  "SHOTS_OV_26.5": "Over 26.5 shots",
  "SHOTS_OV_27.5": "Over 27.5 shots",
  "SHOTS_OV_28.5": "Over 28.5 shots",
  "SHOTS_OV_29.5": "Over 29.5 shots",
  "SHOTS_OV_30.5": "Over 30.5 shots",
  "SHOTS_OV_31.5": "Over 31.5 shots",
};
...
const GRADE_LABEL = {
  ...
  /* Only the three combinations gradeLabel already settles. The side-specific
     ones ("home or over 1.5") and win-either-half are deliberately absent: it
     has no case for them, and writing one here would be the second grader this
     comment warns about. They render as not settled, which is true. */
  "MIX_X_OV_1.5": "Draw or over 1.5", "MIX_X_OV_2.5": "Draw or over 2.5",
  "MIXGG_X": "Draw or both score",
};
...
/* football.com loads a SPORTYBET code and opens the same slip, so it is offered
   beside a SportyBet code and nowhere else. A Bet9ja code means nothing there,
   and a button that fails for half the people who press it is worse than no
   button - which is why this is gated on the book rather than always shown. */
const FOOTBALL = { name: "football.com", url: "https://www.football.com/ng/m?shareCode=" };
...
function legRows(legs) {
  return legs.map((l) => {
    ...
    /* Escaped first, then the teams put in already escaped: a team name is
       attacker-controlled like everything else in the payload. */
    const mkt = esc(MARKETS[l.code] || "")
      .replace("{H}", esc(l.home)).replace("{A}", esc(l.away));
    const meta = [mkt, mark, score].filter(Boolean).join(" ");
    return `
      <li>
...
function renderBody(legs, record, opts) {
  ...
  const head = v.slipWon ? '<p class="vd won">Every game landed 🔥</p>'
    : v.slipLost ? `<p class="vd lost">${v.lost === 1 ? "So close 😤 one leg off." : v.lost + " legs off. We go again 💪"}</p>`
  ...
  const codeCard = code ? `
    <div class="code">
      ...
      </div>${book === BOOKS.sporty ? `
      <p class="also"><a class="fb" id="fb"
         href="${esc(FOOTBALL.url + encodeURIComponent(code))}"
         rel="noopener nofollow" target="_blank">Open in football<span>.</span>com</a></p>` : ""}
    </div>` : "";
...
/* The third way in, and deliberately not a third equal button. Copy and Open
   are the pair people came for; football.com is a smaller pill on its own row,
   in football.com's own purple with their lime on the dot - the same pair the
   app uses, so somebody who has seen one recognises the other. Sized to its
   text rather than stretched, so it reads as an alternative rather than a
   third of the choice. */
.also{margin:11px 0 0;text-align:center}
.also .fb{display:inline-block;font-size:12.5px;font-weight:800;letter-spacing:.01em;
  padding:8px 15px;border-radius:999px;text-decoration:none;
  background:#282450;color:#fff;border:1px solid rgba(159,246,17,.32)}
.also .fb span{color:#9FF611;font-weight:900}
@media (hover:hover){ .also .fb:hover{filter:brightness(1.35)} }
.also .fb:focus-visible{outline:2px solid var(--win);outline-offset:2px}
...
/* The static card is composited at 1568x772 and halved before it ships - see
   the SHRINK note in lib/ogcard.js, where WhatsApp's on-device fetcher turned
   out to give up on the full-size PNG. The per-slip card is served at full
   size. So the two images are different shapes and the declaration has to
   follow the one actually used: this said 1568x772 for both, which was a lie
   on every page that fell back to the static card. */
const SLIP_CARD = { w: 1568, h: 772 };
/* A VERSION ON THE CARD URL, because the card is served immutable for a year.
   The art is composited per request but the BASE is baked, so redrawing the
   design changes what that URL should return while the URL itself stays the
   same - and Cloudflare, told immutable, keeps serving last week's picture to
   every crawler that asks. Measured: after the logo rebake, `?g=21&o=34.32`
   came back 134400 bytes from a cf HIT while the freshly built card was 135189.
   The static og-card.png has carried a ?v= for this reason since September;
   this one had nothing. Derived from the baked base, so a rebake moves it and
   nothing else does. */
const CARD_V = (() => {
  try {
    const fs = require("fs"), path = require("path"), crypto = require("crypto");
    const buf = fs.readFileSync(path.join(__dirname, "..", "assets", "slip-base.z"));
    return crypto.createHash("sha1").update(buf).digest("hex").slice(0, 8);
  } catch (e) { return "0"; }
})();
function shell(o) {
  const url = ORIGIN + (o.path || "/s");
  const card = o.image ? SLIP_CARD : require("./ogcard.js").cardSize();
  return `<!doctype html>
<html lang="en">
<head>
<meta charset="utf-8">
...
${o.noindex === false ? "" : `<meta name="robots" content="noindex,follow">
`}
...
<!-- Spelled out rather than left to the og: fallback. X reads og:title and
     og:description when its own are missing, and usually that is enough - but
     "usually" is the whole problem on a card that has refused to draw four
     times, and two lines cost nothing. -->
<meta name="twitter:title" content="${esc(o.title)}">
<meta name="twitter:description" content="${esc(o.desc)}">
<meta name="twitter:image" content="${esc(o.image || (ORIGIN + "/og-card.png"))}">
<meta name="twitter:image:alt" content="${esc(o.title)} on Soccerwizard">
...
<div class="wrap">
  <a class="top" href="/">
    <img src="/wiz-mark.png" alt="" width="34" height="34">
...
<script>
(function(){
  /* ANDROID CANNOT OPEN THE APP FROM A PLAIN LINK, and this was removed once
     on my say-so and had to come back, so the evidence is written down.
     football.com serves no /.well-known/assetlinks.json, the file Android
     requires before an app may claim a site's https links. I could not confirm
     that from the build machine - football.com block-pages it with a
     CloudFront 403, and so does SportyBet - so I took the workaround out and
     relied on the owner reporting that the link opened the app for him.
     It did: he was tapping the modal in the app, which already used intent://.
     A reader tapping a SHARED slip, which had only the plain URL, landed on
     the website. Two different links, not two different phones.
     intent:// asks Android for the app by package name instead of by URL, and
     browser_fallback_url is what makes it safe: no app, wrong package, or an
     app that declines the link, and the browser opens the same web page it
     would have opened anyway. So the worst case is exactly today's behaviour.
     Applied here rather than in the markup because this page is rendered by a
     server that cannot know the device: the href ships as the plain web URL,
     so iPhone, desktop and anything without JavaScript already work, and
     Android is an upgrade laid on top. */
  var fb=document.getElementById("fb");
  if(fb && /android/i.test(navigator.userAgent)){
    var web=fb.getAttribute("href");
    /* NO REGEX HERE, and no backslash anywhere in this script - including in
       these comments, which is why this one is worded around them.
       Everything between the backticks is a TEMPLATE LITERAL, so JavaScript
       processes escapes on the way out. A regex written here to strip the
       scheme lost both of its escaped slashes, arrived as three slashes in a
       row, terminated early, and turned the rest of the line into a comment.
       A syntax error - and not a quiet one confined to this feature: the
       WHOLE script block failed, so Copy code stopped working on every shared
       slip as well. A plain indexOf cannot be mangled by an escape it does
       not contain. A test compiles what this block emits, and another refuses
       any backslash in it at all. */
    var bare = web.indexOf("://")>=0 ? web.slice(web.indexOf("://")+3) : web;
    fb.setAttribute("href","intent://"+bare+
      "#Intent;scheme=https;package=com.football.app.android"+
...
function renderPage(legs, record, path, opts) {
  ...
  return shell({
    path, title,
    /* NOINDEX IS WHY X DREW NO CARD, and it took a controlled test on X itself
       to see it: the same account, the same composer, thirty seconds apart -
       /how-it-works drew a full card, /s/<code> drew nothing. The only thing
       these pages carry that the static ones do not is noindex, and X honours
       it by refusing to build a preview at all.
       A shared slip still must not be indexed - it is somebody's private
       ticket, thin, and duplicated a thousand times over - so the tag stays
       for search engines and is dropped for the crawlers whose entire job is
       drawing a preview. They do not index anything, so nothing is being
       hidden from a ranking. api/s.js decides, because only the request knows
       who is asking. */
    noindex: opts && opts.noindex,
    ...
    /* SERVED FROM /slipcard.png, NOT /api/slipcard, and the rewrite in
       vercel.json is what makes those the same function.
       robots.txt has to say `Disallow: /api/` - that is where the booking
       routes live - and a crawler reading it refuses the card image, which is
       how a shared slip ended up with a perfect set of meta tags and no
       picture. `Allow: /api/slipcard` fixes that, but only once the crawler
       re-reads the file, and X caches robots.txt for about a day: the Allow
       lands, the cards stay blank, and nothing on our side shows why.
       A path outside /api needs no exception and no cache to expire. It also
       ends the whole class of problem, since every future rule about /api -
       ours, Cloudflare's, anyone's - stops applying to the one URL under it
       that strangers are supposed to fetch. The .png suffix is deliberate too:
       some fetchers decide what to expect from the extension before they see
       a Content-Type. */
    /* `?card=static` swaps this slip's own card for the site's static one.
       It exists because of a real question no amount of reading could answer:
       X draws a card for /how-it-works and none for a slip, and after the tags
       were made identical the only difference left was the picture - a static
       file on one, a function with a query string on the other. This is the
       A/B that tells them apart, and it stays because the same question will
       be asked again the next time a platform goes quiet on us. */
    image: opts && opts.staticCard
      ? `${ORIGIN}/og-card.png`
      : `${ORIGIN}/slipcard.png?g=${encodeURIComponent(n)}` +
        `&o=${encodeURIComponent(t.odds.toFixed(2))}&v=${CARD_V}`,
