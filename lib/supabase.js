...
/* A prediction has no score yet, and the results table was built for finished
   matches - so one of its score columns may refuse the row. Same shape of
   answer as the model column below: recognise the one case, adjust the row,
   write it anyway.
   `hit` becomes false rather than null. It is not a claim: a row written by
   the build carries source "build", confirmScores never trusts one, and it is
   re-graded from a real score before it can reach a page. */
function notNullViolation(why) {
  return /23502/.test(String(why || ""));
}
function withDefaultVerdict(rows) {
  return rows.map((r) => Object.assign({}, r, { hit: r.hit == null ? false : r.hit }));
}
...
async function insertResults(rows) {
  ...
  if (!out.ok && notNullViolation(out.why)) out = await post(withDefaultVerdict(rows));
