...
function renderConvertPage() {
  /* One radius (12px) and one accent (--accent) for the whole page. The brand
     red stays where it already lives, on the wordmark, so the page has a
     single warm accent rather than two competing ones. */
  const css = `
.cv h2{margin:40px 0 14px;font-size:21px;line-height:1.25;letter-spacing:-.01em;
  color:var(--text);text-transform:none;font-weight:800}
.cv h2:first-child{margin-top:28px}
.cv .lede{font-size:17.5px;line-height:1.6;color:var(--text)}
.cv .say{color:var(--soft)}
.cv .cta{margin-top:14px}

/* The three lookups, on a rail. Numbers in the accent, one hairline down the
   left, so the order reads before any of the words do. */
.cv .flow{margin:0;padding:0;list-style:none;border-left:1px solid var(--line)}
.cv .flow li{position:relative;margin:0;padding:0 0 22px 22px}
.cv .flow li:last-child{padding-bottom:4px}
.cv .flow b{display:block;color:var(--text);font-size:15.5px;margin-bottom:3px}
.cv .flow p{margin:0;color:var(--soft);font-size:14.5px;line-height:1.6}
.cv .flow li::before{content:attr(data-n);position:absolute;left:-13px;top:0;
  width:25px;height:25px;border-radius:50%;display:grid;place-items:center;
  background:var(--card2);border:1px solid var(--line);
  color:var(--accent);font-size:12.5px;font-weight:800}

/* What each book sells. Two columns of real lines, because the whole question
   a reader arrives with is "will my leg cross", and that is a comparison. */
.cv .books{margin:0;border:1px solid var(--line);border-radius:12px;
...
.cv .books .c{padding:11px 13px;font-size:14px;line-height:1.45;color:var(--soft);
  border-left:1px solid var(--line)}
.cv .books .c:first-child{border-left:0;color:var(--text);font-weight:600}
.cv .books .hd .c{color:var(--text);font-weight:700;font-size:13.5px}
.cv .books .num{font-variant-numeric:tabular-nums}

/* Four reasons a leg stays behind. Tiles rather than bullets: they are
   independent of each other and none is a step in a sequence. */
.cv .tiles{display:grid;grid-template-columns:1fr 1fr;gap:10px}
.cv .tiles div{border:1px solid var(--line);border-radius:12px;padding:13px 14px;
  background:var(--card)}
.cv .tiles div:nth-child(2n){background:var(--card2)}
.cv .tiles b{display:block;color:var(--text);font-size:14.5px;margin-bottom:4px}
.cv .tiles p{margin:0;color:var(--soft);font-size:14px;line-height:1.55}

/* The two moves, each with the change itself on the line above the reason. */
.cv .move{border:1px solid var(--line);border-radius:12px;background:var(--card);
  padding:14px 15px;margin-top:10px}
.cv .move h3{margin:0 0 9px;font-size:15px;color:var(--text)}
.cv .move p{margin:0;color:var(--soft);font-size:14.5px;line-height:1.6}
.cv .swap{display:flex;flex-wrap:wrap;gap:7px;margin:0 0 10px}
.cv .swap span{border:1px solid var(--line);border-radius:999px;
  padding:4px 11px;font-size:12.5px;color:var(--text);background:var(--card2);
  font-variant-numeric:tabular-nums}
...
/* The price rule, which is the one somebody loses money believing wrongly.
   It had a gold bar down its left edge; a coloured side-tab on a card is a
   house style nobody chose, and the gold is already carrying the numerals and
   the changed chips. The emphasis is the surface and the sentence instead. */
.cv .note{border:1px solid var(--line);border-radius:12px;
  background:var(--card2);padding:14px 15px;margin:0}
.cv .note p{margin:0 0 8px;color:var(--soft)}
.cv .note p:last-child{margin:0}
.cv .note b{color:var(--text)}

/* Markets that cross, as the list of names it is. */
...
.cv .mk span{border:1px solid var(--line);border-radius:999px;padding:5px 11px;
  font-size:13px;color:var(--soft);background:var(--card)}
.cv .mk span.on{color:var(--text);border-color:var(--accent)}

@media (max-width:560px){
  .cv h2{font-size:19px;margin-top:32px}
  ...
  /* The word "Market" over a full-width label row says nothing the label does
     not. The two book names still head their own columns. */
  .cv .books .hd .c:first-child{display:none}
  .cv .books .hd .c:nth-child(2){border-left:0}
}`;

  /* Each row is a line one book quotes and the other may not - read off the
     two PASSTHROUGH_MAPs rather than remembered, and pinned by
     test/convertpage.test.js against the API's own tables. */
  const book = (what, sporty, b9) =>
    `<div class="row"><div class="c">${what}</div>` +
    `<div class="c num">${sporty}</div><div class="c num">${b9}</div></div>`;

  const tile = (head, body) =>
    `<div><b>${head}</b><p>${body}</p></div>`;

  const market = (name, on) =>
    `<span${on ? ' class="on"' : ""}>${name}</span>`;

  return staticPage({
    path: "/convert-a-booking-code",
    title: "Convert a booking code between SportyBet and Bet9ja",
    desc: "Paste a SportyBet or Bet9ja booking code and get the same slip as a " +
      "code for the other bookmaker. What carries over, what is left behind, " +
      "and what a converted slip is worth.",
    h1: "Convert a booking code to the other bookmaker",
    sub: "SportyBet to Bet9ja, and back.",
    ...
    body: `
<div class="prose cv">
  <p class="lede">A booking code only works at the bookmaker that issued it. A
  SportyBet code pasted into Bet9ja is not a slip, it is a typo, and that is
  the most common reason a code "does not load".</p>

  <p class="say">${BRAND} reads the slip behind a code and books the same games
  at the other bookmaker, so what comes back is a code that book will take.</p>

  <a class="cta" href="/">Paste a code and convert it</a>

  <h2>What happens to your slip</h2>
  <ol class="flow">
    <li data-n="1"><b>We ask the bookmaker what is behind the code</b>
      <p>What comes back is what they hold today, which can be shorter than what
      was booked. Both books drop games from a coupon on their own schedule and
      neither says how many there were.</p></li>
    <li data-n="2"><b>Each game is matched to our board</b>
      <p>The board already knows both bookmakers' id for every fixture it
      carries, so nothing is guessed by name at this point.</p></li>
    <li data-n="3"><b>The market is translated and the slip is booked</b>
      <p>At the other book, under its own id for the same bet. You get its own
      code back.</p></li>
  </ol>

  <h2>The same bet, two different cards</h2>
  <p class="say">Most markets exist on both and cross untouched. These are the
  ones where the cards stop in different places:</p>
  <div class="books">
  ...
  <h2>What gets left behind, and why</h2>
  <div class="tiles">
    ${tile("A game that has kicked off",
      "Neither book will take it, and one such leg refuses the whole ticket, so it comes out before we ask.")}
    ${tile("A game the other book does not list",
      "Coverage is not identical. Bet9ja carries about 97% of what SportyBet does.")}
    ${tile("A game that is not on our board",
      "Every board is a day's fixtures, so a code built a week ago will have games we no longer carry.")}
    ${tile("A market only one of them sells",
      "The rows above: a quarter handicap, a 6.5 corner line, a whole Over line. Named on screen, game by game, before you book anything.")}
  </div>

  <h2>The lines that cannot travel unchanged</h2>
  <p class="say">Two families are sold at lines that do not overlap, so the bet
  itself has to move for the leg to cross.</p>

  <div class="move">
    <h3>Whole Over/Under lines</h3>
    ...
    <p>SportyBet sells Over 2 and Over 3; Bet9ja stops at the half lines. Over 2
    returns your stake on exactly two goals where Over 1.5 has already won, so
    these are not the same bet. The leg moves to the nearest line Bet9ja sells
    and every screen that shows it says it was changed.</p>
  </div>

  <div class="move">
    <h3>1X2 or Over/Under</h3>
    <div class="swap"><span>1 or Over 1.5</span><i>becomes</i><span class="to">1 or Over 2.5</span></div>
    <p>Both books sell this at 2.5, so those legs cross untouched. Bet9ja also
    sells 1.5 and 3.5 and SportyBet sells neither, and moving one of those is a
    whole goal, not a half, so it is
    offered with a switch rather than done for you.
    It moves only from Bet9ja to SportyBet, where each of their lines has
  ...
  <h2>What a converted slip is worth</h2>
  <div class="note">
  ...
  <h2>Markets that cross untouched</h2>
  <p class="say">Markets we predict and price:</p>
  <div class="mk">
  ...
  <p class="say">Markets we move but never price, so they carry the bookmaker's
  odds and none of ours:</p>
  <div class="mk">
  ...
  <h2>Reading a code without converting it</h2>
  <p class="say">The same box shows what is behind any code from either book,
  the games, the markets and the bookmaker's own odds, and can cut a long slip
  into several smaller tickets. Reading costs nothing and books nothing.</p>
</div>
<a class="cta" href="/">Paste a code and convert it</a>`,
  });
}
