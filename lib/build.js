...
const SCORE_SOURCES = [
  { name: "soccervista", api: SV },
  { name: "oracle", api: ORACLE },
];

/* Seven, not four. Four was sized to API-Football's free plan, which serves a
   rolling three days; SoccerVista reaches a week, and a week is what it takes
   to still be catching the results a thin sweep left behind. */
...
async function confirmScores(rows, log) {
  ...
  const sources = SCORE_SOURCES.filter((s) => s.api.configured());
  ...
  for (const [date, group] of byDate) {
    /* Whichever source answers first with rows for this date. A source that
       is down, suspended or simply past its window reports a reason and the
       next one is tried, so one dead provider no longer means a blank day. */
    let got = null, from = "";
    for (const src of sources) {
      let r;
      try { r = await src.api.resultsFor(date); }
      catch (e) { r = { ok: false, why: String(e && e.message || e), rows: [] }; }
      if (r.ok && r.rows.length) { got = r; from = src.name; break; }
      log.push(`${src.name} ${date}: ${r.ok ? "no finished matches" : r.why}`);
    }
    if (!got) continue;
    ...
    log.push(`${from} ${date}: ${n} of ${group.length} confirmed from ${got.rows.length} finished`);
  }
  return out;
}

/* Write down what we predicted, while it is still a prediction.
 *
 * A tip only reaches the record if a row exists saying we made it. Until now
 * the only thing writing those rows was the sweep, which notices a match by
 * seeing it live - so a match it never polled was a match we never got credit
 * or blame for. GitHub throttles that sweep to about five runs a day, and on
 * 31 Aug it recorded 21 of roughly 30 tips. The other nine simply never
 * happened as far as the record is concerned.
 *
 * The build has no such problem: it is the thing that decides the tip, so it
 * knows every one of them. This records them at publish time.
 *
 * BEFORE KICK-OFF, and that is the whole point. A tip recomputed after the
 * final whistle is not the tip we showed - the model refits on every build, so
 * an evening rebuild can reach a different call than the morning one did, and
 * a record built from those would be flattering rather than true. So only
 * fixtures that have not started are written, with no score at all. The score
 * arrives later through confirmScores, from a source that watched the match.
 *
 * Insertion is ignore-duplicates, so running twenty times a day writes each
 * prediction once and never overwrites the tip already on file.
 */
async function recordPredictions(fixtures, log) {
  if (!DB.configured()) return;
  const now = Date.now();
  const rows = [];
  for (const f of fixtures || []) {
    if (!f || !f.tip || !f.date || !f.home || !f.away) continue;
    /* Not started. A missing kick-off is treated as started, i.e. skipped -
       we would rather miss a row than record a tip we cannot prove predates
       the result. */
    ...
    rows.push({
      ...
      /* No score, no verdict. Both arrive from confirmScores once a source
         that watched the match can supply them. */
      hg: null, ag: null, hit: null,
  ...
  try {
    ...
    if (out.ok) log.push(`recorded ${out.inserted} new prediction(s) of ${rows.length} upcoming`);
    else log.push(`recording predictions failed (non-fatal): ${out.why}`);
  } catch (e) {
    log.push(`recording predictions failed (non-fatal): ${e.message}`);
  }
}
...
async function buildPayload(options = {}) {
  ...
  await recordPredictions(out, log);
...
const POTD_ASIA = ["Japan","China","India","Indonesia","Malaysia","Singapore",
...
function potdAsian(f) {
  const t = ((f && f.league) || "").trim();
  return POTD_ASIA.some((pre) => t === pre || t.indexOf(pre + " ") === 0);
}
function choosePotd(fixtures, prev, nowMs) {
  ...
  const demerit = (f) => (f.thin ? 1 : 0) + (potdAsian(f) ? 1 : 0);
  const top = pool.slice().sort((a, b) =>
    demerit(a) - demerit(b) || (b.tip_p - a.tip_p) ||
    (potdFid(a) < potdFid(b) ? -1 : 1))[0];
  if (!top) return null;
  return { id: potdFid(top), home: top.home, away: top.away, date: top.date };
}

module.exports = { buildPayload, choosePotd, POTD_ASIA, leanResults, scoreForTip, RESULT_MODEL_KEYS, MAIN, EXTRA, EXTRA_FILE, DEFAULTS,
  confirmScores,
