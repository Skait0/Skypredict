...
async function resultsFor(date, opts) {
  ...
  try {
    ...
    if (!r.ok) return { ok: false, why: "http " + r.status + " " + text.slice(0, 160), rows: [] };
    let body;
    ...
    if (errs && !Array.isArray(errs) && Object.keys(errs).length) {
      return { ok: false, why: Object.values(errs).join("; "), rows: [] };
    }
    return { ok: true, rows: parseFixtures(body) };
  } catch (e) {
