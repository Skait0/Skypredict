  ...
  async function enrichWithFTScores() {
    ...
    if (ORACLE.configured()) {
      for (const d of [0, 1]) {
        ...
        try {
          const got = await ORACLE.resultsFor(date);
          if (got.ok && got.rows.length) oracleRows.set(date, got.rows);
        } catch (e) { log.push(`fixture oracle ${date}: ${e.message}`); }
      }
    }

    log.push(`FT score map built: ${ftMap.size} dated, ${liveFt.size} live, ` +
      `${[...oracleRows.values()].reduce((n, r) => n + r.length, 0)} from the oracle`);
