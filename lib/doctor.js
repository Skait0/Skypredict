...
function findFixture(fixtures, leg) {
  ...
  if (!h || !a) return null;
  return fixtures.find((f) => norm(f.home) === h && norm(f.away) === a &&
    (!ko || !f.kickoff || Math.abs(Date.parse(f.kickoff) - ko) < 3 * 3600e3)) || null;
}
...
function reply(book, code, legs, fixtures, site) {
  ...
  if (!ex.rows.length) {
    ...
    const top = ex.rows.slice(0, 3), low = ex.rows.length > 3 ? ex.rows.slice(-3).reverse() : [];
