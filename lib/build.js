...
async function firstScoreSource(date, log) {
  for (const src of SCORE_SOURCES) {
    ...
    if (r.ok && r.rows.length) return { rows: r.rows, from: src.name };
...
async function confirmScores(rows, log) {
  ...
  for (const [date, group] of byDate) {
    const got = await firstScoreSource(date, log);
    if (!got) continue;
    const from = got.from;
 ...
 */
async function recordPublishedTips(prevFixtures, log) {
  ...
  const byDate = new Map();
  for (const f of fx) {
    /* Played, and long enough ago to have finished. Two hours past kick-off is
       comfortably beyond ninety minutes plus a half-time; anything nearer is
       left for the next build rather than asked about while it is still on. */
    const ko = Date.parse(f.kickoff || "");
    if (!isFinite(ko) || now - ko < 2 * 3600 * 1000) continue;
...
async function buildPayload(options = {}) {
  ...
  await recordPublishedTips(cfg.prevFixtures, log);
