"use strict";
/**
 * SoccerVista's day of finished matches -> rows the model can be fitted on.
 *
 * WHY THIS EXISTS
 *
 * football-data.co.uk carries the history the model is fitted on, and on
 * 5 Sep 2026 it started answering 503 to everyone and had not stopped a week
 * later. The committed floor under data/results keeps the site building - see
 * cachedResultsFor in lib/build.js - but a floor holds only what it held when
 * it was committed, so every day of the outage the board is fitted on form one
 * day older, and nothing about the board looks wrong while that happens.
 *
 * lib/soccervista.js already fetches a thousand-odd finished matches a day,
 * with the score stated rather than inferred, and the site already grades
 * against it. This turns that same feed into the corpus the fit is missing.
 *
 * WHAT MAKES IT SAFE, AND THE ONLY THING THAT DOES
 *
 * A club is a name here, not an id. "Man United" and "Manchester United FC"
 * are two teams to buildIndex, each with half the matches and neither with a
 * usable rating, and nothing downstream would report that - the board would
 * simply be worse. So no name from this feed is ever written as they spell it.
 * Every club is resolved against the index the floor already built, through
 * matchTeam's league-filtered case, and anything that does not resolve is
 * DROPPED and counted. A dropped match costs one result; a wrongly resolved
 * one silently corrupts a rating for a season.
 *
 * That is also why the league mapping below is a written table rather than
 * fuzzy matching. `Spain LaLiga2` is one edit away from `Spain LaLiga`, and
 * a league confused for the division above it would put twenty clubs in the
 * wrong pool at once.
 */
const M = require("./model.js");

/* Their name for a league -> ours, where the two differ. Everything not in
   here has to match our name exactly, which 23 of our 38 leagues already do.
   Written from what the feed actually returned on 1-6 Sep 2026, not from what
   the competitions are called officially.

   Ireland is the trap in this list and is deliberately absent: they publish
   BOTH `Ireland Premier Division` (which is ours, exactly) and `Ireland
   National League`, and guessing which of the two is the League of Ireland
   top flight is exactly the kind of coin-toss this table exists to refuse. */
const LEAGUE_ALIAS = {
  "England League One": "England League 1",
  "England League Two": "England League 2",
  "England National League": "England Conference National",
  "Scotland League One": "Scotland League 1",
  "Scotland League Two": "Scotland League 2",
  "Germany Bundesliga": "Germany Bundesliga 1",
  "Germany 2. Bundesliga": "Germany Bundesliga 2",
  "Spain LaLiga": "Spain La Liga 1",
  "Spain LaLiga2": "Spain La Liga 2",
  "Belgium Jupiler Pro League": "Belgium Pro League",
  "Portugal Liga Portugal": "Portugal Primeira Liga",
  "Brazil Serie A Betano": "Brazil Serie A",
  /* The three warming leagues below. Their canonical names are the ones the
     FIXTURE feed uses, so a harvested result and a fixture to price end up
     under one string - and so the entries in UNRATED_LEAGUES read as the same
     league they gate. */
  "Denmark 1st Division": "Denmark 1. Division",
  "Norway OBOS-ligaen": "Norway 1st Division",
  /* countryOf() reads the first word of a league name, so a country whose own
     name is two words files under "Czech" or "Bosnia". These three are
     renamed to a single-word country; every other league below already
     starts with one. */
  "Czech Republic Chance Liga": "Czechia Chance Liga",
  "Bosnia and Herzegovina WWIN Liga BiH": "Bosnia WWIN Liga",
  /* The second wave, 23 Sep - see the second HARVEST_EXTRA block. Ours is
     SportyBet's spelling, because that is the label a fixture arrives with
     and `f.league in idx.lIdx` is an exact match. SoccerVista spells nine of
     them differently, down to a lower-case l. */
  "Brazil Serie B": "Brazil Brasileiro Serie B",
  "Slovakia 2. liga": "Slovakia 2. Liga",
  "Bulgaria Vtora liga": "Bulgaria Vtora Liga",
  "Italy Serie C - Group A": "Italy Serie C, Group A",
  "Italy Serie C - Group B": "Italy Serie C, Group B",
  "Italy Serie C - Group C": "Italy Serie C, Group C",
  "Spain Primera RFEF - Group 1": "Spain Primera Federacion",
  "Spain Primera RFEF - Group 2": "Spain Primera Federacion",
  "Colombia Primera A": "Colombia Liga DIMAYOR",
  "Uruguay Liga AUF Uruguaya": "Uruguay Primera Division",
  /* SportyBet files the Meistriliiga under its sponsor's name, so every
     Estonian fixture arrived as a league we do not rate. */
  "Estonia Premium Liiga": "Estonia Meistriliiga",
  /* The third wave's SoccerVista spellings, read off their board 19-23 Sep,
     so these keep harvesting after the paid API-Football month ends. */
  "Ecuador Liga Pro": "Ecuador LigaPro Primera A",
  "Paraguay Copa de Primera": "Paraguay Division de Honor",
  "Brazil Serie C": "Brazil Brasileiro Serie C",
  "Northern Ireland NIFL Premiership": "Northern Ireland Premiership",
  "Northern Ireland NIFL Championship": "Northern Ireland Championship",
};

/* LEAGUES WE HARVEST BUT HAVE NO HISTORY FOR.
 *
 * football-data never carried these, so there is nothing to seed them with and
 * nothing to backfill from - SoccerVista's board reaches about a week back.
 * They start at zero and gain a round a week.
 *
 * That is fine, and it needs no switch. lib/build.js drops any league with
 * fewer than cfg.minLeagueMatches results before fitting, so a warming league
 * sits in the harvest file, stays out of the model on its own, and joins the
 * board the week it crosses the line - roughly seven or eight rounds, so about
 * two months from the first harvest.
 *
 * The only thing that cannot be recovered later is the calendar: a week not
 * harvested is a week that never enters the corpus. That is the whole reason
 * to start collecting a league before it is of any use. */
/* LEAGUES WHERE RESERVE SIDES ARE MEMBERS - Jong Ajax, Stuttgart II,
 * Atletico Madrid B, PAOK B - playing the full season against senior clubs.
 * Here, and only here, a reserve side may be minted as a club of its own;
 * everywhere else it is refused as before. Counted from API-Football's
 * 2023-24 seasons: without them the Eerste Divisie lost 140 of 380 games a
 * season. Add a league only once its reserve members have been seen. */
const RESERVE_LEAGUES = new Set([
  "Netherlands Eerste Divisie", "Germany 3. Liga", "Spain Primera Federacion",
  "Italy Serie C, Group A", "Italy Serie C, Group B", "Italy Serie C, Group C",
  "Austria 2. Liga", "Greece Super League 2", "Bulgaria Vtora Liga",
  "Slovakia 2. Liga",
]);
const HARVEST_EXTRA = [
  "Netherlands Eerste Divisie",
  "Denmark 1. Division",
  "Norway 1st Division",

  /* THE REASON THE BOARD IS THIN ON EUROPEAN NIGHTS.
   *
   * A cross-border tie needs BOTH clubs in the index, and we rate 27 countries
   * against the fifty-odd that reach the UEFA competitions. So PSG v Slovan
   * Bratislava, PSV v Shakhtar Donetsk, Slavia Prague v Lens and Man Utd v
   * Sabah are all dropped as "no league" - 650 fixtures on the board of
   * 8 Sep 2026, and the European ones are the fixtures people come looking for.
   *
   * These are the top flight of every UEFA country we did not rate and
   * SoccerVista carries. Same warming rule as the three above: they sit in the
   * harvest, stay out of the fit until they cross minLeagueMatches, and switch
   * themselves on. About two months at a round a week, and there is no
   * shortcut - their board reaches seven days back, so this history cannot be
   * bought, only accumulated. The clock starts the day they are listed here,
   * which is the whole reason to list them before they are useful.
   *
   * ONE MORE THING IS NEEDED BEFORE A EUROPEAN TIE PRICES. countryHandicap in
   * lib/build.js refuses a country with no UEFA_COEFFICIENT entry, and it
   * refuses before it ever looks at the fitted offset. Adding these leagues is
   * necessary and not sufficient; the coefficients are a separate, deliberate
   * data change, due before the first of these crosses 60 matches. */
  "Czechia Chance Liga",
  "Slovakia Nike liga",
  "Ukraine Premier League",
  "Croatia HNL",
  "Serbia Mozzart Bet Super Liga",
  "Israel Ligat ha'Al",
  "Cyprus Cyprus League",
  "Bulgaria efbet League",
  "Slovenia Prva liga",
  "Hungary NB I.",
  "Azerbaijan Premier League",
  "Kazakhstan Premier League",
  "Georgia Crystalbet Erovnuli Liga",
  "Iceland Besta deild karla",
  "Belarus Vysshaya Liga",
  "Bosnia WWIN Liga",
  "Latvia Virsliga",
  "Lithuania TOPLYGA",
  "Estonia Meistriliiga",
  "Armenia Premier League",
  "Albania Abissnet Superiore",
  "Moldova Super Liga",
  "Malta Premier League",
  "Luxembourg BGL Ligue",
  "Wales Cymru Premier",
  "Montenegro Prva Crnogorska Liga",
  "Kosovo Superliga",

  /* THE SECOND WAVE: WHAT "NO LEAGUE" TURNED OUT TO BE.
   *
   * The block above was listed for the European nights, and once
   * scripts/apifbackfill.js gave it history those ties priced. What was left
   * on 23 Sep 2026 was 609 fixtures dropped as "no league", and measured by
   * competition they were not European at all: a hundred national-team games,
   * and three hundred in domestic second and third tiers - Serie C, the
   * Primera Federacion, the 3. Liga, the Primera Nacional, Superettan.
   *
   * These are the ones with a single table (or a handful of groups) and a
   * season of fixtures on the card. The fourth and fifth tiers - Segunda
   * Federacion's five groups, Sweden's six Division 2s - are left out on
   * purpose: a lot of requests for thin, churning divisions.
   *
   * Same rules as the first wave: warming until they cross minLeagueMatches,
   * then they switch themselves on. apifbackfill.js is what gets them there
   * inside a day rather than inside a season. */
  "Germany 3. Liga",
  "France Ligue 3",
  "Italy Serie C, Group A",
  "Italy Serie C, Group B",
  "Italy Serie C, Group C",
  "Spain Primera Federacion",
  "Brazil Brasileiro Serie B",
  "Argentina Primera Nacional",
  "Austria 2. Liga",
  "Greece Super League 2",
  "Slovakia 2. Liga",
  "Bulgaria Vtora Liga",
  "Sweden Superettan",
  "Mexico Liga de Expansion MX",
  "USA USL Championship",
  "Colombia Liga DIMAYOR",
  "Uruguay Primera Division",
  "Chile Primera Division",

  /* THE THIRD WAVE, 25 Sep 2026, with the paid API-Football month. Tiers one
   * to three that the board dropped as "no league" on 24 Sep. The fourth tier
   * and below stay out for the reason above - Segunda Federacion, Sweden's
   * Division 2, Denmark's 3rd Division, the National League North and South -
   * because a paid plan fixes the request count, not the thin data. Names are
   * SportyBet's spelling. Northern Ireland files under its whole name via
   * model.js countryOf, not under "Northern". */
  "Northern Ireland Premiership",
  "Northern Ireland Championship",
  "Ecuador LigaPro Primera A",
  "Paraguay Division de Honor",
  "Canada Canadian Premier League",
  "Guatemala Liga Nacional",
  "Japan J2 League",
  "Ireland First Division",
  "Finland Ykkosliiga",
  "Russia 1. Liga",
  "Egypt 2. Division A",
  "Brazil Brasileiro Serie C",
  "Argentina Primera B",
  "Denmark 2nd Division",
];

/* Their league name -> ours, but only if we hold ratings for it. `allowed` is
   the set of league names the build fits on; anything else is not a mapping
   failure, it is a competition we correctly do not model. */
function ourLeague(theirs, allowed) {
  const name = LEAGUE_ALIAS[theirs] || theirs;
  return allowed && allowed.has(name) ? name : null;
}

/* Rows from lib/soccervista.js `resultsFor`, plus the date they were asked
   for, resolved into matches in OUR spelling.
 *
 * Returns the matches and a tally of why rows were let go, because the tally
 * is the only warning available if their naming drifts: a table that has
 * quietly stopped matching shows up as a resolution rate, never as an error. */
/* Which league indices belong to each country, by the convention every league
   name in this codebase follows: "England Championship", "Spain La Liga 2". */
function leaguesByCountry(idx) {
  const by = new Map();
  idx.leagues.forEach((name, li) => {
    const country = M.countryOf(name);
    if (!by.has(country)) by.set(country, []);
    by.get(country).push(li);
  });
  return by;
}

/* A CLUB THAT HAS MOVED DIVISION IS STILL THE SAME CLUB.
 *
 * matchTeam is league-filtered, and the index gives a club the league of its
 * most recent match - which, with football-data down, is last season's. So on
 * 5-6 Sep 2026 every promoted and relegated side failed to resolve: Sheffield
 * Wednesday and Leicester in League One, Girona and Mallorca in La Liga 2,
 * eight of Serie B. 65 of 258 in-scope rows, and all of them clubs we have
 * years of history for.
 *
 * So a miss in the named league is retried across that country's other
 * divisions - the same widening lib/euroresolve.js does, with the same rule:
 * a UNIQUE hit or nothing. The whole-index search is never used. Two clubs
 * called Rangers in different divisions is exactly the coincidence that would
 * put a second-tier result on a top-flight club's rating.
 *
 * The match is then recorded under the league the FEED named, not the one the
 * index remembers, because the feed is describing this season and the index is
 * describing last one. That is also how the promotion reaches the index at
 * all: buildIndex takes a club's league from its newest match. */
function findClub(idx, name, league, byCountry) {
  const li = idx.lIdx[league];
  if (li != null) {
    const direct = M.matchTeam(idx, name, li);
    if (direct) return direct;
  }
  const country = M.countryOf(league);
  const hits = new Set();
  for (const other of (byCountry.get(country) || [])) {
    if (other === li) continue;
    const t = M.matchTeam(idx, name, other);
    if (t) hits.add(t);
    if (hits.size > 1) return null;
  }
  return hits.size === 1 ? [...hits][0] : null;
}

/* A LEAGUE WITH NO HISTORY CANNOT RESOLVE ITS OWN CLUBS.
 *
 * findClub asks the index, and for a warming league the index holds nothing -
 * so the first harvest of Netherlands Eerste Divisie dropped all 22 rows and
 * would have dropped them every week forever. The league can never start.
 *
 * So for HARVEST_EXTRA leagues only, a club that resolves nowhere is taken at
 * the feed's spelling and entered as a new club. That is the only way one of
 * these leagues ever gets a first row.
 *
 * Two things keep it honest. findClub still runs FIRST, so a side relegated
 * out of a league we do rate is recognised as itself rather than minted a
 * second time under another spelling. And a youth, reserve or women's side is
 * refused outright: matchTeam guards those, minting would not, and this is the
 * one path that skips matchTeam. */
/* NAMES THAT WOULD COLLIDE IN THE INDEX, RENAMED AT SOURCE - FOR EVERY FEED.
 * normName strips "Club", so Uruguay's "Club Nacional" folds to "nacional",
 * the same key as Madeira's Nacional, and matchTeam - which needs a unique hit
 * - then answers neither. The same table joins one club spelled two ways
 * across seasons (Aue, Viktoria Koln). It lived in scripts/apifbackfill.js
 * until 24 Sep 2026, when the SoccerVista harvest brought its own "Club
 * Nacional" and broke the Madeira alias again; it is applied in resolve() now,
 * so no feed can reintroduce a name the table has already settled. Keyed by
 * our league name, never by the name alone. */
const LEAGUE_RENAME = {
  "Uruguay Primera Division|Club Nacional": "Nacional Montevideo",
  "Colombia Liga DIMAYOR|Fortaleza FC": "Fortaleza CEIF",    // not Brazil's Fortaleza
  "Germany 3. Liga|Erzgebirge AUE": "Erzgebirge Aue",
  "Germany 3. Liga|FC Viktoria Koln": "FC Viktoria Köln",
  /* API-Football's spellings in the extra top flights, for the statistics
     overlay (25 Sep 2026). Scoped to the league, so "Dynamo" is Dynamo Moscow
     only inside the Russian Premier League. China's clubs are renamed often:
     these are the same clubs under their current names. */
  "China Super League|Chengdu Better City": "Chengdu Rongcheng",
  "China Super League|Meizhou Kejia": "Meizhou Hakka",
  "China Super League|Dalian Zhixing": "Dalian Yingbo",
  "China Super League|Qingdao Jonoon": "Qingdao Hainiu",
  "China Super League|Qingdao Youth Island": "Qingdao West Coast",
  "Romania Superliga|Metaloglobus": "Metaloglobus Bucharest",
  "Romania Superliga|Rapid": "FC Rapid Bucuresti",
  "Romania Superliga|Universitatea Craiova": "Univ. Craiova",
  "Romania Superliga|CFR 1907 Cluj": "CFR Cluj",
  "Romania Superliga|Csikszereda": "Csikszereda M. Ciuc",
  "Brazil Serie A|Flamengo": "Flamengo RJ",
  "Brazil Serie A|Botafogo": "Botafogo RJ",
  "Brazil Serie A|Vasco DA Gama": "Vasco",
  "Russia Premier League|Dynamo": "Dynamo Moscow",
  "Russia Premier League|Lokomotiv": "Lokomotiv Moscow",
  "Russia Premier League|Akhmat": "Akhmat Grozny",
  "Russia Premier League|Rubin": "Rubin Kazan",
  "Japan J1 League|Urawa": "Urawa Reds",
  "Japan J1 League|Kashima": "Kashima Antlers",
  "Argentina Liga Profesional|Belgrano Cordoba": "Belgrano",
  "Argentina Liga Profesional|Union Santa Fe": "Union de Santa Fe",
  "Argentina Liga Profesional|Central Cordoba de Santiago": "Central Cordoba",
  "Poland Ekstraklasa|Nieciecza": "Termalica B-B.",
  "Sweden Allsvenskan|AIK Stockholm": "AIK",
  "Austria Bundesliga|WSG Wattens": "Tirol",
  "Ireland Premier Division|St Patrick's Athl.": "St. Patricks",
  "Mexico Liga MX|U.N.A.M. - Pumas": "UNAM Pumas",
};
function renamed(league, name) { return LEAGUE_RENAME[league + "|" + name] || name; }

function resolve(rows, idx, allowed, date, bootstrap) {
  const matches = [];
  const dropped = { league: 0, club: 0, malformed: 0 };
  const byCountry = leaguesByCountry(idx);
  const canStart = bootstrap instanceof Set ? bootstrap : new Set(bootstrap || []);
  for (const r of (rows || [])) {
    const league = ourLeague(r && r.league, allowed);
    if (!league) { dropped.league++; continue; }
    /* A warming league is absent from the index by definition, which is not a
       reason to drop its rows - it is the state it has to pass through. Any
       other league missing from the index is one we hold no ratings for. */
    if (idx.lIdx[league] == null && !canStart.has(league)) { dropped.league++; continue; }
    if (!r.home || !r.away || r.home === r.away ||
        !Number.isFinite(r.hg) || !Number.isFinite(r.ag)) {
      dropped.malformed++; continue;
    }
    /* A reserve side is minted only in a league where reserve sides are
       members (RESERVE_LEAGUES), and only when it is a reserve side and not a
       youth or women's one - M.reserveBase refuses those. matchTeam places it
       afterwards by its parent club, never onto the parent itself. */
    const mint = (name) => {
      const t = String(name || "").trim();
      if (!canStart.has(league) || !t) return null;
      if (!M.isVariantSide(t)) return t;
      return (RESERVE_LEAGUES.has(league) && M.reserveBase(t)) ? t : null;
    };
    const rh = renamed(league, r.home), ra = renamed(league, r.away);
    const home = findClub(idx, rh, league, byCountry) || mint(rh);
    const away = findClub(idx, ra, league, byCountry) || mint(ra);
    if (!home || !away || home === away) { dropped.club++; continue; }
    const m = { date, league, home, away, hg: r.hg, ag: r.ag };
    for (const [, k] of STATS) if (r[k] != null) m[k] = r[k];
    matches.push(m);
  }
  return { matches, dropped };
}

/* The generic layout lib/model.js normalise() already reads, so nothing in the
   build has to learn a new shape. Quoted throughout: a club with a comma in it
   would otherwise silently shift every column right of it. */
const HEADER = "date,league,home_team,away_team,home_goals,away_goals";

function q(s) { return '"' + String(s == null ? "" : s).replace(/"/g, '""') + '"'; }

/* Match statistics, written only when some row carries them (API-Football's
   backfill), so every other live file keeps its six columns. model.js
   normalise reads these by name in the generic layout. */
const STATS = [["home_corners", "hc"], ["away_corners", "ac"], ["home_shots", "hs"],
  ["away_shots", "as"], ["home_sot", "hst"], ["away_sot", "ast"]];

function toCSV(matches) {
  const withStats = (matches || []).some((m) => STATS.some(([, k]) => m[k] != null));
  const lines = [HEADER + (withStats ? "," + STATS.map(([h]) => h).join(",") : "")];
  for (const m of (matches || [])) {
    const extra = withStats ? STATS.map(([, k]) => m[k]) : [];
    lines.push([m.date, m.league, m.home, m.away, m.hg, m.ag].concat(extra).map(q).join(","));
  }
  return lines.join("\n") + "\n";
}

/* Reading our own file back, so a harvest can be merged into the one before it
   rather than replacing it. Deliberately tolerant of a missing or short file -
   the first run has nothing to merge with. */
function fromCSV(text) {
  const rows = M.parseCSV(String(text || ""));
  if (rows.length < 2) return [];
  const res = M.normalise(rows);
  if (res.error) return [];
  return res.matches.map((m) => {
    const out = {
      date: m.date instanceof Date ? m.date.toISOString().slice(0, 10) : String(m.date),
      league: m.league, home: m.home, away: m.away, hg: m.hg, ag: m.ag,
    };
    for (const [, k] of STATS) if (m[k] != null) out[k] = m[k];
    return out;
  });
}

/* One match, one row. The same tie can arrive twice - a harvest overlapping the
   previous one, or their feed listing a late kick-off under both dates - and a
   duplicated result is a match the fit counts twice. */
function dedupe(matches) {
  const seen = new Set(), out = [];
  for (const m of (matches || [])) {
    const k = `${m.date}|${m.league}|${m.home}|${m.away}`;
    if (seen.has(k)) continue;
    seen.add(k);
    out.push(m);
  }
  out.sort((a, b) => (a.date < b.date ? -1 : a.date > b.date ? 1 :
    a.league < b.league ? -1 : a.league > b.league ? 1 :
    a.home < b.home ? -1 : a.home > b.home ? 1 : 0));
  return out;
}

module.exports = { LEAGUE_ALIAS, HARVEST_EXTRA, RESERVE_LEAGUES, LEAGUE_RENAME, renamed, ourLeague, resolve, findClub, toCSV, fromCSV, dedupe, HEADER };
