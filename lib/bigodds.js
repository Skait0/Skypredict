...
async function mint(pay, now) {
  try {
    const body = await (await fetch(SITE() + "/api/fixtures")).json();
    const p = pick(pay, now, C.events("sporty", body));
    if (!p) return null;
    const sporty = await C.convert(p.legs, "sporty", now);
    if (!sporty.code) return null;
    const codes = { sporty: { code: sporty.code, n: sporty.booked.length } };
    for (const to of ["bet9ja", "betking", "betpawa"]) {
      const r = await C.convert(p.legs, to, now);
      if (r.code) codes[to] = { code: r.code, n: r.booked.length };
    }
    return Object.assign(texts(p, codes), { row: { day: S.dayOf(now), odds: Number(p.odds.toFixed(2)), codes,
      legs: p.legs.map((l) => ({ home: l.home, away: l.away, code: l.prediction, p: l.p, odds: l.odds })) } });
  } catch (e) { return null; }
}

module.exports = { pick, texts, mint, TIP_CODE };
