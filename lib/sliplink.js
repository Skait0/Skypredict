...
function renderBody(legs, record) {
  ...
  const rows = legs.map((l) => {
    /* null is "not settled", never a loss. */
    const mark = l.won === true ? '<span class="sl-w">won</span>'
      : l.won === false ? '<span class="sl-l">lost</span>' : "";
    const score = (l.hg != null && l.ag != null)
      ? `<span class="sl-sc">${esc(l.hg + " - " + l.ag)}</span>` : "";
    return `
    <li class="sl-leg">
      <div class="sl-teams">${esc(l.home)} <span class="sl-v">v</span> ${esc(l.away)}</div>
      <div class="sl-pick">${[esc(MARKETS[l.code]), mark, score].filter(Boolean).join(" ")}</div>
      <div class="sl-nums"><b>${esc(l.od.toFixed(2))}</b><span>${Math.round(l.p * 100)}%</span></div>
    </li>`;
  }).join("");

  const v = verdict(legs);
  const head = v.slipWon
    ? '<p class="sl-verdict sl-w">Every leg landed.</p>'
    : v.slipLost
      ? `<p class="sl-verdict sl-l">${v.lost} leg${v.lost === 1 ? "" : "s"} missed, so the slip is down.</p>`
      : (v.won || v.lost)
        ? `<p class="sl-verdict">${v.won} landed so far, ${v.unknown} still to play.</p>`
        : "";
...
.sl-verdict{font-size:14px;font-weight:700;margin:0 0 14px}
.sl-w{color:#3ddc84}
.sl-l{color:var(--red,#bc0f2b)}
.sl-leg .sl-w,.sl-leg .sl-l{font-size:11px;font-weight:800;text-transform:uppercase;letter-spacing:.05em}
.sl-sc{font-size:12px;color:var(--faint);font-variant-numeric:tabular-nums}`;
