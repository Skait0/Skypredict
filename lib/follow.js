...
function labelFor(code) {
  ...
  /* Half lines only. A whole line (Over 2) is refunded on exactly two goals,
     and gradeLabel's strict > would call that push a loss - the one thing a
     grader must never guess. Left untracked, and said so. */
...
/* Safe for Telegram's HTML mode. The board itself carries some names already
   entity-encoded ("Bosnia &amp; Herzegovina"), so decode first or the reader
   sees "&amp;"; and one raw "&" or "<" makes Telegram refuse the whole message,
   which left a follow retrying forever (code review, 24 Sep). */
function html(s) {
...
function step(follow, results, now) {
  ...
  legs.forEach((l, i) => {
    ...
    if (won == null) return;
    seen[i] = won ? "won" : "lost";
    lines.push((won ? "✅ " : "😤 ") + html(l.name) + " · " + html(r.home) + " " + r.hg + "-" + r.ag + " " + html(r.away));
  });
