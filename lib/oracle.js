...
const ALIAS = { utd: "united", sth: "south", nott: "nottingham", st: "saint",
  ...
  /* Our Scottish feed writes "Raith Rvs"; bookmakers write "Raith Rovers". */
  rvs: "rovers" };
...
function parseFixtures(body) {
  ...
  for (const f of (body && body.response) || []) {
    ...
    out.push({
      ...
      status: String(st),
      /* Their fixture id, so the statistics for a matched row can be asked
         for by id afterwards (statsFor). */
...
/* One fixture's statistics block as our columns: corners hc/ac and total
   shots hsh/ash. API-Football lists each side under its team id; a missing or
   null value is a number we do not have, never a zero. Null when the corners
   are not both there, since those are what a leg needs. */
function statsOfFixture(f) {
  ...
  const out = { hc: v(h, "Corner Kicks"), ac: v(a, "Corner Kicks"),
                hsh: v(h, "Total Shots"), ash: v(a, "Total Shots") };
  return out.hc == null || out.ac == null ? null : out;
}

/* Statistics for up to twenty finished fixtures in ONE request - /fixtures
   takes `ids` joined by "-" and returns each fixture with its statistics
   inline (verified 25 Sep 2026). Returns { ok, stats: {id: {...}|null}, quota }. */
async function statsFor(ids, opts) {
  const list = (ids || []).filter((x) => x != null).slice(0, 20);
  if (!configured()) return { ok: false, why: "not configured", stats: {} };
  if (!list.length) return { ok: true, stats: {} };
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), (opts && opts.timeoutMs) || TIMEOUT_MS);
  try {
    const r = await fetch(BASE + "/fixtures?ids=" + list.join("-"), {
      signal: ctrl.signal, headers: { "x-apisports-key": key() } });
    const left = r.headers && r.headers.get("x-ratelimit-requests-remaining");
    const quota = left == null ? null : Number(left);
    const body = await r.json().catch(() => null);
    if (!r.ok || !body) return { ok: false, why: "http " + r.status, stats: {}, quota };
    const errs = body.errors;
    if (errs && !Array.isArray(errs) && Object.keys(errs).length) {
      return { ok: false, why: Object.values(errs).join("; "), stats: {}, quota };
    }
    const stats = {};
    for (const f of body.response || []) {
      if (f && f.fixture && f.fixture.id != null) stats[f.fixture.id] = statsOfFixture(f);
    }
    return { ok: true, stats, quota };
  } catch (e) {
    return { ok: false, why: String((e && e.message) || e), stats: {} };
  } finally {
    clearTimeout(t);
  }
}
...
/* Is a paid plan active? /status does not count against the allowance, so a
   build can ask every time. Same test as .github/workflows/apif-refresh.yml:
   active and a daily limit above the free plan's 100. Anything that goes
   wrong answers "not paid", which leaves the build on the free-plan limits -
   the safe way round. Returns { paid, limit }. */
async function planStatus(opts) {
  if (!configured()) return { paid: false, limit: null };
  const ctrl = new AbortController();
  const t = setTimeout(() => ctrl.abort(), (opts && opts.timeoutMs) || TIMEOUT_MS);
  try {
    ...
    const res = r.ok ? ((await r.json()) || {}).response || {} : {};
    const limit = Number((res.requests || {}).limit_day) || null;
    return { paid: !!(res.subscription || {}).active && limit > 100, limit };
  } catch (e) {
    return { paid: false, limit: null };
  } finally {
    clearTimeout(t);
  }
}
...
module.exports = {
  configured, resultsFor, planStatus, findMatch, parseFixtures, markers, sameVariant,
  similarity, toks, sameTok, FINAL, ninety, statsFor, statsOfFixture,
};
