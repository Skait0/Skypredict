...
const BOOKS = {
  ...
  betking: "/api/betking/booking-code",
  betpawa: "/api/betpawa/booking-code",
};
...
function makeHandler(opts) {
  ...
  return async (req, res) => {
    ...
    /* ?check=lines - SportyBet's live card read for the slip's corners and
       shots legs BEFORE booking (API /api/sporty/live-check). Books nothing,
       so it neither asks nor spends the quota. A failure answers "nothing
       found" and the page books as it always did. */
    if (req.query && req.query.check === "lines") {
      if (which !== "sporty") return res.status(200).json({ verdicts: [] });
      const up = await postUpstream("/api/sporty/live-check", payload);
      return res.status(200).json(up.ok && up.body && Array.isArray(up.body.verdicts)
        ? { verdicts: up.body.verdicts } : { verdicts: [] });
    }
    ...
    /* ONE LINE PER BOOKING, SO ATTRIBUTION CAN BE READ. The src rides in the
       query string, and Vercel's log export (`vercel logs`) drops query
       strings - three days of src= were collected and nothing outside the
       dashboard could read them (24 Sep 2026). This line lands in the
       request's own logs. Only src is echoed, and only as a short lowercase
       word; anything else is "other". */
    if (process.env.VERCEL) {
      const src = require("./bookgate.js").srcOf(req) || "none";
      console.log(JSON.stringify({ sw: "book", book: which, src: src, status: out.status,
        legs: payload.selections.length }));
    }
