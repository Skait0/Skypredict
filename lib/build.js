...
function outcomeHit(label, hg, ag, hth, hta) {
  const tot = hg + ag, home = hg > ag, draw = hg === ag, away = ag > hg, btts = hg > 0 && ag > 0;
  switch (label) {
    case "Home win": return home;
    case "Draw": return draw;
    case "Away win": return away;
    case "Over 2.5": return tot >= 3;
    case "Under 2.5": return tot <= 2;
    case "Both teams score": return btts;
    case "First half goal": return (hth != null && hta != null) ? (hth + hta >= 1) : null;
    case "1X, home or draw": return home || draw;
    case "12, anybody wins": return !draw;
    case "X2, draw or away": return draw || away;
    case "Draw or over 2.5": return draw || tot >= 3;
    case "Draw or both score": return draw || btts;
    case "Both score and over 2.5": return btts && tot >= 3;
  }
  return null;
}
...
async function buildPayload(options = {}) {
  ...
  // fit
  const t0 = Date.now();
  const model = M.fitModel(matches, { halfLife: cfg.halfLife, reg: cfg.shrinkage });
  ...
  try {
    ...
    const nowD = new Date();
    const train = matches.filter((m) => m.date < cutoff);
    const test = matches.filter((m) => m.date >= cutoff && m.date <= nowD);
    if (train.length > 500 && test.length >= 20) {
      const hIdx = M.buildIndex(train);
      const hModel = M.fitModel(train, { halfLife: cfg.halfLife, reg: cfg.shrinkage, index: hIdx });
      let correct = 0, total = 0;
      for (const m of test) {
        const p = M.predictTotals(hModel, m.home, m.away, m.league);
        if (!p) continue;
        const k = M.markets(p, { fhShare: fhShare[m.league], k: hModel.k });
        const tip = M.bestTip(k);
        const hit = outcomeHit(tip.label, m.hg, m.ag, m.hth, m.hta);
        if (hit === null) continue;
        total++; if (hit) correct++;
      }
      if (total >= 20) record = { correct, total, days: N };
    }
  } catch (e) { /* record stays null */ }
