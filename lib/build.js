...
function scoreForTip(k, tipLabel, seed) {
  ...
  /* Draw from the BULK of the distribution, not its tail.
     Sampling the whole distribution is right in aggregate - the board's
     goal spread matched real results almost exactly - and wrong on any
     single line, because a tail cell presented as "the prediction" reads as
     nonsense however sound the ensemble is. It published 4-3 for a fixture
     the model gives 2.85 expected goals, and 3-3 for Napoli v Como at 2.47.
     Nobody reading one match cares that the tail is correctly sized.
     So the cells are ranked and taken until they cover COVER of the
     probability, and the sampling happens inside that. Variety is
     untouched - the bulk still holds a dozen or more scorelines - while
     the outcomes nobody would defend for THIS match stop appearing. */
  /* 0.80 measured against real results, not chosen. Looser cuts push mass
     into four-goal lines - 19% at 0.88 and 22% at 0.94 against 12% in real
     results - without meaningfully recovering the five-goal tail, and they
     bring back scorelines the fixture cannot justify: 0.94 published a 4-2
     on a 2.82-goal game.
     A consequence worth stating: games of five or more goals are 18% of real
     results and about 4% of the board. That gap is honest rather than a
     miscalibration. A single published scoreline can be a typical outcome
     for its match or it can chase the tail across the card; it cannot do
     both, and a reader looking at one fixture is owed the former. */
  const COVER = 0.80;
  const ranked = weighted.slice().sort((a, b) => b.p - a.p);
  const mass = ranked.reduce((t, c) => t + c.p, 0);
  const bulk = [];
  let acc = 0;
  for (const c of ranked) {
    bulk.push(c);
    acc += c.p;
    if (mass > 0 && acc / mass >= COVER) break;
  }
