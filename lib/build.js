...
/* The scoreline we publish beside a tip must be one the tip actually survives.
   The modal cell of the joint distribution is not that: for a strong home
   favourite it is routinely 1-1, so the card said "predicted 1-1" next to
   "tip: Home win" on 10 of 357 fixtures. Both numbers were right and the pair
   was nonsense.
   Walk the ranked shortlist and take the likeliest scoreline that the real
   grader - the same one that settles finished matches - calls a win for this
   tip. That makes the scoreline mean "the most likely way this tip comes in",
   which is both coherent and more use than a bare mode.
   Falls back to the mode when nothing in the shortlist settles the tip, which
   is right for markets a scoreline cannot decide at all (first-half goals, for
   instance): better an honest mode than an invented scoreline. */
/* The scoreline the page has always shown: goal count rounded from the two
   expected-goal figures, nudged to agree with the over/under call, then split
   in proportion and pushed off a draw toward whoever the model favours.
   It is a summary rather than the modal cell of the distribution, and that is
   why it reads well - the total moves with the fixture, so a 3.9-goal game
   looks different from a 2.4-goal one. Kept because it works: across 357
   fixtures it spreads over five scorelines with none above 45%, where picking
   the likeliest coherent cell instead piles 62% onto 2-1. */
...
function scoreForTip(k, tipLabel) {
  if (!k || !tipLabel) return k && k.score;

  /* The house scoreline first. It disagreed with its own tip on 1 fixture in
     357 - Alaves v Villarreal, two evenly matched sides where rounding pushed
     an X2 tip onto a home win - and replacing all 357 to fix that one made
     the board far worse than the flaw did. So use it whenever it holds, and
     only reach for something else on the rare case where it does not. */
  const house = houseScoreline(k);
  if (house) {
    const [h, a] = house.split("-").map(Number);
    if (GRADE.gradeLabel(tipLabel, h, a) !== false) return house;
  }
  if (!Array.isArray(k.scores)) return house || k.score;

  /* Surviving the tip is necessary and not sufficient.
     Taking the first scoreline the tip merely survives printed 1-1 on 87% of
     the card: a draw satisfies both 1X and X2, two goals clears Over 1.5, and
     1-1 is the modal cell for most fixtures - so it won almost every time.
     Coherent and useless, and a home favourite reading "1-1" is worse than the
     contradiction it replaced.
     So prefer a scoreline that also agrees with the model's own lean. Where
     the model favours the home side, the scoreline it publishes should be a
     home win; where it genuinely favours the draw, 1-1 is the honest answer
     and still gets chosen. Falls back to tip-survival alone when the lean has
     no scoreline the tip allows - the coherence guarantee is never traded. */
  const lean = (k.home >= k.draw && k.home >= k.away) ? "home"
             : (k.away >= k.draw && k.away >= k.home) ? "away" : "draw";
  const agrees = (h, a) =>
    lean === "home" ? h > a : lean === "away" ? a > h : h === a;

  /* Among the scorelines that qualify, prefer the one whose goal count sits
     nearest what the model expects from this fixture.
     Ranking purely by probability picked 1-0 for 58% of the card, because the
     modal decisive scoreline is 1-0 almost everywhere - true, and no more
     use to a reader than a board of 1-1 was. Expected goals is the number
     that actually differs between fixtures: a game priced at 3.1 goals should
     not be summarised as 1-0 when 2-1 is right there and says something.
     Probability still breaks ties, so nothing improbable is reached for. */
  const want = (k.lh != null && k.la != null) ? k.lh + k.la : null;
  let best = null, bestKey = null, survives = null;
  for (const c of k.scores) {
    const [h, a] = c.s.split("-").map(Number);
    if (GRADE.gradeLabel(tipLabel, h, a) !== true) continue;
    if (!survives) survives = c.s;             // best that at least holds
    if (!agrees(h, a)) continue;
    if (want == null) return c.s;              // no expectation to fit
    const key = Math.abs((h + a) - want);
    if (bestKey === null || key < bestKey - 1e-9) { bestKey = key; best = c.s; }
  }
  return best || survives || k.score;
}
...
async function buildPayload(options = {}) {
  ...
  for (const f of fixtures) {
    ...
    out.push({
      ...
      total: Math.round(k.total * 100) / 100, score: scoreForTip(k, tip.label),
  ...
  for (const m of matches) {
    ...
    results.push({ date: m.date.toISOString().slice(0, 10), league: m.league,
      ...
      lh: rr(kk.lh), la: rr(kk.la), total: rr(kk.total), score: scoreForTip(kk, tp.label),
    ...
    if (got.ok && got.rows.length) {
      ...
      for (const r of got.rows) {
        ...
        results.push(Object.assign({ date: r.match_date, league: r.league || "",
          ...
          tip: r.tip, hit: !!r.hit, recorded: true },
          (r.model && typeof r.model === "object") ? r.model : {}));
        added++;
      }
      if (added) {
        results.sort((a, b) => (a.date < b.date ? 1 : -1));
        log.push(`${added} result(s) filled in from the live-score record`);
      }
...
module.exports = { buildPayload, leanResults, scoreForTip, RESULT_MODEL_KEYS, MAIN, EXTRA, EXTRA_FILE, DEFAULTS,
  ...
  LEAGUE_ALIASES, canonLeague, isUnratedCompetition };
