...
/* Only ask about dates a source can actually answer for. Both feeds serve a
   rolling window and refuse the rest, and a refusal still costs a request;
   anything older is the results feed's job. */
function inScoreWindow(date) {
  const t = Date.parse(date + "T00:00:00Z");
  return isFinite(t) && (Date.now() - t) <= ORACLE_WINDOW_DAYS * 86400000;
}
...
/* Only ask a source about a date it can actually answer for. */
function sourceCanAnswer(src, date, budget) {
  if (src.name !== "oracle") return true;
  if (budget.spent >= ORACLE_BUDGET) return false;
  const t = Date.parse(date + "T00:00:00Z");
  return isFinite(t) && (Date.now() - t) <= ORACLE_MAX_AGE_DAYS * 86400000;
}
...
async function firstScoreSource(date, log, budget, sources) {
  ...
  if (budget.memo.has(date)) return budget.memo.get(date);
  ...
  for (const src of (sources || SCORE_SOURCES)) {
    ...
    if (!sourceCanAnswer(src, date, budget)) {
      if (src.name === "oracle") {
        log.push(`oracle ${date}: not asked (${budget.spent >= ORACLE_BUDGET
          ? "budget of " + ORACLE_BUDGET + " spent this build"
          : "older than its " + ORACLE_MAX_AGE_DAYS + "-day window"})`);
      }
      continue;
    }
    ...
    if (r.ok && r.rows.length) { out = { rows: r.rows, from: src.name }; break; }
    log.push(`${src.name} ${date}: ${r.ok ? "no finished matches" : r.why}`);
  }
  /* Cached either way. A date with no result is still an answer, and asking a
     second time would cost a second request to be told the same thing. */
  budget.memo.set(date, out);
  return out;
}
...
function logScoreBudget(budget, log) {
  ...
  log.push(`oracle: ${budget.spent} of ${ORACLE_BUDGET} calls used, ` +
    `${budget.memo.size} date(s) looked up` +
...
async function confirmScores(rows, log, budget) {
  const out = new Map();
  ...
  for (const r of fresh) {
    if (!inScoreWindow(r.match_date)) { stale++; continue; }
  ...
  for (const [date, group] of byDate) {
    const got = await firstScoreSource(date, log, budget);
    if (!got) continue;
    const from = got.from;
...
async function recordPublishedTips(prevFixtures, log, budget) {
  ...
  for (const f of fx) {
    ...
    if (!inScoreWindow(f.date)) continue;
  ...
  for (const [date, group] of byDate) {
    ...
    for (const f of group) {
      const m = ORACLE.findMatch(got.rows, f.home, f.away);
      if (!m) continue;
...
async function buildPayload(options = {}) {
  ...
  try {
    ...
    if (got.ok && got.rows.length) {
      ...
      const verified = await confirmScores(pending, log, scoreBudget);
  ...
  await recordPublishedTips(cfg.prevFixtures, log, scoreBudget);
...
module.exports = { buildPayload, choosePotd, POTD_TIER_COST, potdScore, leanResults, scoreForTip, RESULT_MODEL_KEYS, MAIN, EXTRA, EXTRA_FILE, DEFAULTS, recordPublishedTips,
  carryInPlay, IN_PLAY_MAX_MS,
  firstScoreSource, makeScoreBudget, logScoreBudget, sourceCanAnswer,
