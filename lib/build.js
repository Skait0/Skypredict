 ...
 *
 * That yields 28 distinct scorelines, 33% draws and 22% low-scoring games -
 * a board shaped like the football it is describing.
 *
...
function scoreForTip(k, tipLabel, seed) {
  ...
  const total = pool.reduce((t, c) => t + c.p, 0);
  ...
  for (const c of pool) { r -= c.p; if (r <= 0) return c.s; }
