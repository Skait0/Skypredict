 ...
 */
/* `half` carries what a final score cannot: the interval score as hth/hta, and
   the fixture's home/away names for the markets whose labels say a team rather
   than a side. Absent, those markets return null. */
function gradeLabel(label, hg, ag, half) {
  ...
  /* WIN EITHER HALF. The model has published this market since it was graded
     into the proven set, and nothing here could settle it: the label names a
     team rather than a side, so an unrecognised string fell through to null
     and every one of these tips came back ungraded.
     It needs two things a final score does not carry - the half-time score,
     and which end of the fixture the named team was at - so the caller passes
     both. Without either it is genuinely unknowable, which is null, not a
     miss. The second half is the full-time score minus the interval, the same
     arithmetic gradeEveryMarket does at build time. */
  let wh = /^(.+?) to win a half$/i.exec(t);
  if (wh) {
    if (!half || half.hth == null || half.hta == null) return null;
    const name = wh[1].trim();
    const isHome = half.home && name === String(half.home).trim();
    const isAway = half.away && name === String(half.away).trim();
    if (!isHome && !isAway) return null;
    const h1h = half.hth, h1a = half.hta, h2h = hg - half.hth, h2a = ag - half.hta;
    return isHome ? (h1h > h1a || h2h > h2a) : (h1a > h1h || h2a > h2h);
  }
  ...
  /* The side-specific combinations and team totals, added 24 Sep 2026 for
     the Telegram bot's "Follow my slip": tipsters' slips lean on them, and a
     second grader somewhere else is the thing this file exists to prevent. */
  m = /^(Home|Away) win or over\s*([\d.]+)/i.exec(t);
  if (m) return (m[1].toLowerCase() === "home" ? diff > 0 : diff < 0) || tot > parseFloat(m[2]);
  m = /^(Home|Away) win or both score/i.exec(t);
  if (m) return (m[1].toLowerCase() === "home" ? diff > 0 : diff < 0) || both;
  m = /^(Home|Away) over\s*([\d.]+)/i.exec(t);
  if (m) return (m[1].toLowerCase() === "home" ? hg : ag) > parseFloat(m[2]);
  if (/^Not both teams score/i.test(t)) return !both;
...
function marketOf(label) {
  ...
  /* The name gradeEveryMarket writes at build time, so the live record and the
     backtest land on the same line instead of two half-filled ones. */
  if (/ to win a half$/i.test(t)) return "Win either half";
  ...
  /* One side's goals: "Levante over 0.5 goals". These start with a team name,
     so every "Over" rule above misses them and they fell into "Other" - which
     is not published. The market was switched on by default on 31 Aug, became
     the model's strongest call on 27 of 34 fixtures that day, and was being
     served to people while carrying zero graded results on a site whose whole
     claim is that every tip gets checked. Both sides share one line, because
     "the weaker team to score" and "the stronger team to score" are the same
     bet in different clothes and splitting them halves an already thin
     sample. */
  m = /\bover\s*([\d.]+)\s*goals?$/i.exec(t);
  if (m) return "Team over " + m[1];
  return "Other";
}
