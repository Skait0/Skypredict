...
function xCodePost(e) {
  ...
  return "🧙 Free booking code for " + xDay(e.date) + "\n\n" + books.join("\n") + "\n\n" +
...
function xResultPost(e, resultOf) {
  ...
  const tail = "\n\nEvery game, graded: " + ORIGIN + codesDayPath(e.date);
...
function renderXPosts(entries, resultOf) {
...
<span class="meta">${xLen(text)} / 280</span></p></section>`;
  ...
  const body = (code ? card("Today's code - " + xDay(code.date), xCodePost(code))
