...
function pageFooter() {
  return `<footer>
    <p class="links"><a href="/">Predictions</a> &middot; <a href="/matches">All matches</a> &middot; <a href="/how-it-works">How it works</a> &middot; <a href="/privacy">Privacy</a> &middot; <a href="/terms">Terms</a> &middot; <a href="mailto:${CONTACT}">Contact</a> &middot; <a href="${TELEGRAM}" rel="noopener">Telegram</a></p>
...
/* A LASTMOD THAT IS TRUE, WHICH IS THE ONLY KIND WORTH SENDING.
 *
 * Every one of the 933 URLs carried the build date, and the build runs daily,
 * so the sitemap claimed the whole site changed every day - including match
 * pages for games played three weeks ago, whose result has not moved since the
 * final whistle. Google's stated response to an unreliable lastmod is to stop
 * believing the field at all, so the one signal we have for "this page is new,
 * come and look" was being spent on a claim that was false for most of the file.
 *
 * An entry may be a plain path, which takes the build date, or { path, lastmod },
 * which carries its own. A played match uses the day it was played: that page is
 * the result and our tip against it, and neither moves again. An upcoming fixture
 * keeps the build date, because its prediction really is rebaked every deploy.
 *
 * A malformed date falls back to the build date rather than being written out -
 * an invalid lastmod would cost us the whole file, not one line. */
